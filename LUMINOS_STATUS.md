...
| Server OS install | 📋 Blocked on user | [CHANGE: claude-code \| 2026-07-29] Laptop is a **Dell** — `F2` setup, `F12` boot menu. **Two hard warnings recorded in MEDIA_SERVER_PLAN §7a:** (1) disabling Secure Boot (required — neither the Arch ISO nor systemd-boot is signed) can trigger a **BitLocker recovery prompt** on the owner's Windows, so `manage-bde -status C:` and suspend/back up the key *first*; (2) **never** flip `SATA Operation` from `RAID On` to `AHCI` — it stops Windows booting. Still blocked on writing the ISO to a USB stick, which **erases the NTFS stick** currently holding a movie for the TV. |
| State migration | 📋 Pending | `/var/lib/{jellyfin,sonarr,prowlarr}` + `/etc/jellyfin` must be rsync'd off the G14 rather than rebuilding the library, indexers and watch state by hand. |
