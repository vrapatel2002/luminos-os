...
| `/usr/local/bin/dgpu-exec-v2` (`root:dgpu`, mode `2755`) | dGPU gate that also `setresgid`s, so the `dgpu` group is **real**, not just effective — required or bash wrappers drop it and Chrome's setuid sandbox refuses to start. Source: `scripts/dgpu-gate/dgpu-exec-v2.c`. Installed **alongside** v1 `dgpu-exec`, which everything except `chrome-luminos` still uses. | DECISION 52 / BUG-102. Do not build in `/tmp` (`nosuid` ignores setgid). [CHANGE: claude-code \| 2026-08-05] |
