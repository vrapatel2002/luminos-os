...
int main(int argc,char**argv){
  ...
  VkDeviceSize bytes=(VkDeviceSize)THREADS*4;
  ...
  for(uint32_t i=0;i<THREADS;i++) ((float*)ptr)[i]=1.0f;

  FILE*f=fopen("fma.spv","rb"); if(!f){perror("fma.spv");return 1;}
  ...
  printf("PEAK FP32   : %.3f TFLOPS\n",flops_per_run/best/1e12);
  printf("MEAN FP32   : %.3f TFLOPS\n",flops_per_run/(sum/RUNS)/1e12);
  return 0;
}
