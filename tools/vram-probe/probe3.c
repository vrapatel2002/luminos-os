...
int main(int argc,char**argv){
  ...
  int forceType = argc>2 ? atoi(argv[2]) : -1;
  ...
  if(forceType>=0){
    if(!(mr.memoryTypeBits&(1u<<forceType))){ fprintf(stderr,"type %d not allowed for this buffer\n",forceType); return 1; }
    mi=forceType;
  } else {
  ...
  printf("memory type : %u (heap %u, flags 0x%x)\n", mi, mp.memoryTypes[mi].heapIndex, mp.memoryTypes[mi].propertyFlags);
  ...
  if(mp.memoryTypes[mi].propertyFlags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT){
