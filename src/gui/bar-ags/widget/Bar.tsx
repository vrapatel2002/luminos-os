...
export default function Bar(gdkmonitor: Gdk.Monitor) {
  ...
  const briIcon = createPoll("display-brightness-symbolic", 5000, getBrightnessIcon)
  const briText = createPoll("", 5000, () => {
    const b = getBrightness()
    return b >= 0 ? `${b}%` : ""
  })
  const volIcon = createPoll("audio-volume-high-symbolic", 5000, () => getVolumeIcon(getVolume()))
