...
// [CHANGE: gemini-cli | 2026-04-26] Expanded to include rainbow-cycle, rainbow-wave, and speed for highlight/laser
// [CHANGE: gemini-cli | 2026-04-26] Stripped down to 1-Zone hardware realities for GA403UU
...
// GA403UU supported: static, pulse, highlight, laser, breathe, rainbow-cycle, rainbow-wave
static const QStringList COLOR_ONLY_EFFECTS = {QStringLiteral("static"), QStringLiteral("pulse"), QStringLiteral("highlight"), QStringLiteral("laser")};
static const QStringList SPEED_ONLY_EFFECTS = {QStringLiteral("rainbow-cycle")};
static const QStringList FULL_EFFECTS       = {QStringLiteral("breathe"), QStringLiteral("rainbow-wave")};
...
class LuminosKeyboardKcm : public KQuickManagedConfigModule
{
    ...
    Q_PROPERTY(bool hasDirection READ hasDirection NOTIFY modeChanged)
...
public:
    ...
    bool hasColor()     const { return COLOR_ONLY_EFFECTS.contains(m_mode) || m_mode == QStringLiteral("breathe"); }
    bool hasColor2()    const { return m_mode == QStringLiteral("breathe") && m_breatheDual; }
    bool hasSpeed()     const { return m_mode == QStringLiteral("breathe") || SPEED_ONLY_EFFECTS.contains(m_mode) || m_mode == QStringLiteral("rainbow-wave"); }
    bool hasDirection() const { return m_mode == QStringLiteral("rainbow-wave"); }
    ...
    void setDirection(const QString &v) { if (m_direction != v) { m_direction = v; Q_EMIT directionChanged(); setNeedsSave(true); } }
    ...
    void load() override {
        ...
        if (f.open(QIODevice::ReadOnly)) {
            ...
            while (!in.atEnd()) {
                ...
                if (key == QLatin1String("KB_DIRECTION"))     m_direction  = val;
        ...
        if (m_direction.isEmpty()) m_direction = QStringLiteral("Right");
        ...
        Q_EMIT modeChanged(); Q_EMIT speedChanged(); Q_EMIT directionChanged(); Q_EMIT breatheDualChanged(); Q_EMIT hasColor2Changed();
    ...
    void defaults() override {
        ...
        setSpeed(1);
        setDirection(QStringLiteral("Right"));
...
private:
    ...
    void applyToHardware() {
        ...
        } else if (m_mode == QStringLiteral("highlight") || m_mode == QStringLiteral("laser")) {
            // ONLY color flag -c, NO --speed flag for highlight and laser
            args << QStringLiteral("-c") << m_color;
        } else if (m_mode == QStringLiteral("static") || m_mode == QStringLiteral("pulse")) {
            args << QStringLiteral("-c") << m_color;
        } else if (m_mode == QStringLiteral("rainbow-cycle")) {
            args << QStringLiteral("--speed") << speedName();
        } else if (m_mode == QStringLiteral("rainbow-wave")) {
            args << QStringLiteral("--speed") << speedName()
                 << QStringLiteral("--direction") << m_direction.toLower();
        }
    ...
    QString     m_direction      = QStringLiteral("Right");
