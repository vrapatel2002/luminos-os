...
- Status: **PARTIALLY FIXED, still OPEN** as of 2026-08-29. The nodes now come up `0660 root:dgpu` at
  boot and the CUDA risk that blocked the fix is disproven, but a root-privileged `nvidia-smi` still
  re-opens them while the dGPU is awake. Root cause is fully identified — see Finding 3.
...
- **What is still open:** the node is re-opened to 0666 whenever a root NVIDIA client runs — see
  Finding 3. Closing that needs `luminos-power` to drop privileges for its read-only `nvidia-smi`
  polls (`cmd/luminos-power/main.go:1513,1546,1607`); its control calls (`-pl`, `-pm`,
  `--lock-gpu-clocks`) genuinely need root and would still reset the nodes, though far less often.
  **Not done in this pass on purpose:** `luminos-power` is the fan and thermal manager, and swapping
  that binary unattended is a worse risk than the defence-in-depth gap it would close.
- **Two mechanisms evaluated and rejected**, recorded so they are not re-tried:
  ...
  2. *Dropping the setuid bit from `/usr/bin/nvidia-modprobe`.* This genuinely works — it blocks the
     unprivileged reset path, and CUDA still reached the card through the gate in testing. Reverted
     anyway: it does not stop the root poller, so on its own it buys nothing, and it diverges a
     pacman-managed driver binary that would need a hook to survive every `nvidia-utils` upgrade.
...
- **The second, rarer mechanism, also proven today.** `vkCreateSwapchainKHR` returns
  ...
  `dxgi_vk_swap_chain_recreate_swapchain_in_present_task`, and vkd3d retries it ~100×/second for
  as long as the game runs — there is no give-up path — so the first failure is fatal and the run
  only *looks* alive. vkd3d ships with its log off, which is why nothing ever reached
