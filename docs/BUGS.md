...
<!-- [CHANGE: claude-code | 2026-08-28] -->
- Status: **OPEN** — documented and now detected, deliberately not changed. See "why not fixed yet".
...
- **Why not fixed yet:** tightening the UVM nodes is a live-service risk, not a free win. HIVE's
  `llama-server` needs CUDA, and CUDA needs UVM — if that process does not already go through the
  gate, gating UVM takes the HIVE models off the GPU. That needs verifying against a running HIVE
  before touching it, and it was outside the scope agreed for this pass.
