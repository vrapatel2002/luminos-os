...
- Proposed test, to be run when the user is present and can log back in (do NOT run unattended,
  ...
  journalctl -b -u luminos-uvm-gate               # want "created" not "already present"
  ...
  ```
  The journal line matters more than the perms do: **"created" means we won the race, "already
  present" means something beat us to it and we only fixed it up afterwards** — which happens to
  give the right answer at boot but proves the ordering is wrong and would fail on a slower boot.
