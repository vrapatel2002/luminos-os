...
that kills it. That is the whole remaining search space.

1. **Try other Proton builds — cheapest, and never once attempted.** Only
   `GE-Proton10-34` is installed (`ls ~/.local/share/Steam/compatibilitytools.d/`
   returns exactly one entry). A different Proton ships a different vkd3d-proton,
   which emits different SPIR-V. ~5 min per build via `007-try.sh`.
2. Make the current vkd3d emit differently: lower `VKD3D_FEATURE_LEVEL`, or bisect
   `VKD3D_CONFIG` codegen flags.
3. Identify the specific shader. `VKD3D_SHADER_DUMP_PATH` writes every DXIL/SPIR-V
   module; the last one written before the fault is the culprit, and it would make
   a minimal reproducer.
4. Report it upstream. A crash address inside `libnvidia-glvkspirv.so` plus a
