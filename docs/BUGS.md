...
Last Updated: 2026-08-29 (BUG-149 **BUG-142 WAS NEVER A vkd3d BUG — WE DELETED A DEVICE NODE AND DID NOT NOTICE.** `007 First Light` now runs on the RTX 4050 with **zero** swapchain failures, minutes at 98-100% GPU and 58-70 W, never reaching the 780M fallback, where the same build produced 278 failures in 3 s hours earlier. Cause: BUG-146 stripped the setuid bit from `/usr/bin/nvidia-modprobe` (correct, and staying) — but that setuid helper was the **only** thing on this machine creating `/dev/nvidia-modeset`, because `60-nvidia.rules` runs `nvidia-modprobe` and `nvidia-modprobe -c0 -u` and **never passes `-m`**. The node stopped existing and nothing logged it. Found on the first post-reboot health sweep, not by reasoning: `dgpu-exec-v2 --check` listed `/dev/nvidia-modeset absent` as a neutral line while printing `access: OK / gate: OK`, and `vulkaninfo` through the gate died on `vkGetPhysicalDeviceDisplayPropertiesKHR failed with ERROR_UNKNOWN`. **The control was run and is what made it evidence:** the same `vulkaninfo` on AMD enumerates the 780M cleanly, so the error is NVIDIA-specific, not a headless/Wayland artefact. `nvidia-modprobe -m` restored the node — which came back `root:dgpu 660` **unaided**, so the `NVreg_DeviceFile*` params do cover this one unlike the UVM pair — and the ERROR_UNKNOWN vanished on the next run. **Honest limit:** the call proven fixed is `vkGetPhysicalDeviceDisplayPropertiesKHR`; the call 007 died on is `vkCreateSwapchainKHR`. Same presentation path, different entry point, and the link between them is empirical (the game works) not traced — if the storm ever returns *with the node present*, BUG-142 reopens. Fixed in `luminos-uvm-gate.sh`, which now runs `/usr/bin/nvidia-modprobe -m` when the node is absent and then verifies the permissions rather than trusting the driver param; it runs as root from both the udev rule and the backstop service, which is exactly why the gate can still do this when an ordinary application no longer can. Absent-and-uncreatable is now a loud failure naming the BUG-142 symptom, so nobody spends another two days on vkd3d. `dgpu-exec-v2 --check` was **also** fixed: an absent node used to print `absent` and count as nothing, which is precisely how this hid — it now counts as a problem and names BUG-149. Three lessons worth more than the fix: **removing a privilege removes capabilities you did not enumerate** (the BUG-146 notes correctly listed the nodes we *had* covered and never asked which others that binary was creating — the answer was one, and it cost two days); **a green health check is only as wide as what it checks**; and **run the control** — `vulkaninfo` failing on NVIDIA meant nothing until it was run on AMD and did not fail. The storm detector stays: it is now the regression alarm for this. Not yet done: the delete-and-recreate test was deliberately skipped while the game was live and is folded into the next reboot check. Earlier: BUG-147 **BOOT RACE CLOSED** — **the first real reboot proved the UVM gate had been losing its race at every single boot, and the fix it was built around would have bricked the card had it ever worked.** The gate's own fail-loudly instrumentation, added hours earlier, caught it: `UNGATED NODE FOUND: /dev/nvidia-uvm was [root:root 666]` at t=9.10 s. Cause found, not guessed — `60-nvidia.rules` runs `nvidia-modprobe -c0 -u` on the nvidia PCI `add|bind` uevent at **t=6.87 s**, during initramfs-time module load, and a `multi-user.target` oneshot cannot run until **t=9.09 s**. A **2.2 second world-open window, every boot**; `nvidia-persistenced`, `nvidia-powerd` and early KMS were all named as suspects and are all innocent. **The far worse finding came from testing the alternative fix:** `TEST!="/dev/nvidia-uvm"` in `60-nvidia.rules` guards **both** of its `RUN+=` lines, so pre-creating the UVM node — the script's stated design goal, and its `created ... won the race` success message — suppresses `nvidia-modprobe` *entirely* and `/dev/nvidia0` and `/dev/nvidiactl` are never created. Measured: main nodes deleted with UVM present, `add` trigger restored **neither**, `nvidia-smi` failed with "couldn't communicate with the NVIDIA driver". **The gate has only ever worked because it LOST.** Fixed by `config/udev/71-luminos-uvm-gate.rules`, which rides the same PCI uevent and runs `luminos-uvm-gate --udev` immediately after `nvidia-modprobe`. The script's claim that "a udev rule cannot work here" was true only of a rule keyed on the *node* (`mknod(2)` emits no uevent, which is why `70-luminos-dgpu-access.rules` has never fired for anything) — a rule keyed on the *PCI device* works, and ordering was proven by a marker script that observed the nodes as `root:root 666` rather than ABSENT. Window **2.2 s → 9-10 ms** (`nvidia-uvm`) and **18 ms** (`-uvm-tools`), 3 runs; 5/5 delete+trigger cycles gated unaided; backstop service green; mknod guard verified to REFUSE and exit 1 when `/dev/nvidia0` is absent. The window is **not zero** and the docs must not claim it is. Consequences: the service is now a BACKSTOP not the fix; the healthy boot journal is a lone `gated:` line and **`created` is now the alarming outcome, the exact opposite of what the checklist said**; `--udev` mode suppresses the loud path only where repair *is* the job; and `raced`/`refused` are tracked separately after a single folded flag produced the flatly false message "the nodes are correct NOW" while both nodes were absent. Separately, the same reboot's 007 launch confirmed **BUG-142 is alive and the new storm detector works**: it fired twice on the RTX 4050 (278 and 235 failures in 12x250 ms, 10/12 buckets active) and fell back to the 780M, which rendered. The storm ran at **~93/s**, not the 13.1/s measured the day before — the bug has at least two speeds, and the detector survived only because it counts failures-in-window rather than matching a rate. Earlier: BUG-148 **FIXED** — **the ~2-minute wait at every poweroff was one of our own daemons deadlocking in its own SIGTERM handler.** `hive-daemon.py` *did* install a handler, which is why nothing looked wrong, but that handler called `server.shutdown()` — a method that blocks until the `serve_forever()` loop acknowledges, while Python delivers signals **on the main thread**, i.e. the very thread stuck inside `serve_forever()`. It waited for a loop that could not run until it returned. systemd then waited out the full 90 s `DefaultTimeoutStopSec` and SIGKILLed it, at every single shutdown. Found by diffing consecutive monotonic timestamps across eight boots' shutdown sequences: exactly **one** gap over 3 s existed in any complete record, and it was **90.06 s**, ended by `luminos-hive.service: State 'stop-sigterm' timed out. Killing.` Reproduced in isolation before touching anything, and all four candidates measured: direct call **HUNG** >10 s; `shutdown()` from a thread **0.51 s** (shipped); `sys.exit(0)` 0.11 s and `os._exit(0)` 0.10 s — both **rejected**, because they are faster precisely by *skipping* the orderly shutdown rather than performing it (in-flight requests undrained, socket unclosed), and draining is the whole point of the call. Verified live: the old-code instance hung and was killed (`result=timeout`, `9/KILL`), the fixed one stopped in **0.17 s** with `result=success`, and three further cycles ran 0.52/0.16/0.18 s. `TimeoutStopSec=20` added as insurance so the *next* hang costs 20 s not 90 s — a bound on the damage, not the fix. Checked and it is **not** a pattern: the other two Python daemons with SIGTERM handlers set a flag, which is correct. Side finding: `luminos-hive.service` was **not in git** at all (same class as `007-run.sh` and `nvidia-egl-priority.hook`) — now tracked. Earlier: BUG-146 **FIXED** — **the UVM nodes were not just created world-open at boot, they are re-opened at runtime by our own thermal daemon, and the first root cause we wrote down was only half of it.** The world-opening has TWO independent causes and a fix for either one alone changes nothing, which is why the first fix plan looked complete and was not. Measured all four combinations on 2026-08-29 (repair the nodes, run one `nvidia-smi --query-gpu=power.draw`, stat the nodes): non-root caller + setuid `nvidia-modprobe` = gate LOST; non-root caller + setuid stripped = gate HELD; root caller + setuid = LOST; **root caller + setuid stripped = still LOST**. So the setuid-root helper re-opens the nodes on behalf of an unprivileged caller, *and* a euid-0 caller re-opens them without the helper. Fixed in three parts: (1) the read-only polls in `cmd/luminos-power/main.go` now run through `nvidiaQuery()`, which drops to `nobody:dgpu` via `SysProcAttr.Credential`; (2) setuid is stripped from `/usr/bin/nvidia-modprobe` and kept stripped by `config/pacman-hooks/luminos-nvidia-modprobe-setuid.hook`, because pacman restores the bit on every `nvidia-utils` upgrade and a one-time `chmod` reverts silently; (3) four `nvidia-smi` calls change hardware state and therefore **cannot** drop root — `-pm`, `-pl`, `--lock-gpu-clocks`, `--reset-gpu-clocks` — so they go through `nvidiaCtl()`, which re-asserts `root:dgpu 0660` immediately afterwards. Part 3 is not belt-and-braces: with setuid already stripped, a single root `nvidia-smi -pl 55` was measured to reset both nodes on its own. Verified with a 40 s soak across the startup `-pl 55`: nodes held `0660 root:dgpu` throughout, a gated client saw the RTX 4050 and opened `/dev/nvidia-uvm`, an ungated client was denied. Side finding: `--query-gpu=power.limit` returns `[N/A]` on this card even as root, so `initGPUTGP` has always fallen through to `setGPUTGP(55)` — pre-existing, unrelated, now documented. Cost: `nvidia-modprobe` can no longer load modules or create nodes on demand, which is only safe because `/etc/modules-load.d/nvidia.conf` and `luminos-uvm-gate.service` do both up front — **if either is removed the pacman hook must be removed with it.** An audit *inode watch* (`auditctl -w`, after a syscall rule with `-F path=` failed to see `fchmod()` on an open fd) caught `nvidia-smi` running as root from `luminos-power.service` doing `chmod 0666` then `chown root:root` on `/dev/nvidia-uvm`. Any NVIDIA client with euid 0 re-applies the driver's device-file defaults, and for UVM those are hardcoded — `nvidia-modprobe --help` confirms no permission option exists. Four theories died first: nvidia-modprobe's create path, supergfxd, devtmpfs auto-creation, and a plain chmod. The **CUDA fear that kept this bug open is disproven**: `open(2)` checks the *effective* gid and HIVE's `llama-server` already goes through the gate, so with UVM at `0660 root:dgpu` a full Dolphin-8B load ran at `--n-gpu-layers 99` with 4890 MiB resident and answered a prompt, while ungated CUDA stayed correctly blind. Boot-time fix shipped as `luminos-uvm-gate.service`, which **pre-creates** both nodes correctly owned — `nvidia-modprobe` only mknods a node that is absent, so winning the race is enough — reading the major from `/proc/devices` because the kernel allocates it dynamically. A paired `.path` unit was written, tested, and **removed**: `PathExists=` stays satisfied while the path exists and this service creates rather than consumes it, so it spun ~4x/second into systemd's start limit; it was also pointless, because devtmpfs nodes survive `rmmod` with permissions intact. Dropping setuid from `nvidia-modprobe` was tried on 2026-08-28, worked, and was reverted with "it does not stop the root poller, so alone it buys nothing" — **that reasoning was wrong**, and the same objection applies equally to the privilege drop it was rejected in favour of. Neither half works alone; both are now shipped. See BUG-146's Finding 4 for the truth table, and read Finding 4 before Finding 3 — Finding 3 was close enough to sound right and produced a fix plan that does not work. v1 of the gate is now RETIRED from all live callers: `hive-start-model.sh` and `luminos-gpu-yield` both launch `llama-server` through `dgpu-exec-v2`, verified end-to-end — HIVE's nexus model came up with `Gid: 948 948 948 948` (the `setresgid` signature) holding 5718 MiB on the RTX 4050 and answered a question. Earlier: BUG-145 FIXED — **NVIDIA Vulkan was not "dead system-wide"; the dGPU gate was killed by the very change that made it correct.** `/etc/environment` pins the box to the Mesa EGL vendor (the BUG-046c/BUG-050 fix), and libglvnd reads that variable with `secure_getenv(3)`, which returns NULL when `AT_SECURE` is set. v1 of the gate raised only the *effective* gid, so every target ran `AT_SECURE=1`, the Mesa pin was **ignored**, and NVIDIA worked — by accident, because the machine's own graphics policy did not apply to a privileged process. v2's `setresgid` made real==effective, `AT_SECURE` went to 0, the pin was finally **obeyed**, and NVIDIA's Vulkan ICD — which lives inside `libGLX_nvidia.so.0`, a glvnd library, which is why an *EGL* variable kills *Vulkan* — was discarded with `Found no drivers!`. So **v1 never supported NVIDIA properly and must not be restored**: it is the version that silently hands the iGPU to every shell-launched app (BUG-102). The variable is present in `environ` in all three cases — the kernel does not strip it — so grepping the environment could never have shown this; a 12-line C probe printing `getauxval(AT_SECURE)` is what cracked it. Fixed by having the gate set the NVIDIA vendor environment itself *after* the credential change, saying out loud what v1 got by accident: values already naming `nvidia` are left alone (idempotent, caller-overridable), a Mesa value is replaced, and if no NVIDIA vendor JSON exists the variable is **unset** rather than pointed at a missing file. New `dgpu-exec-v2 --check` prints identity, predicted target `AT_SECURE`, node perms, EGL vendor, ICD and power state with a plain verdict. BUG-144 FIXED — the **AMD** branch of the GPU picker was *also* broken, for a completely unrelated reason: `luminos-gpu-launch:65` pointed `VK_ICD_FILENAMES` at `radeon_icd.x86_64.json`, a filename that has never existed on Arch. `VK_ICD_FILENAMES` is an override, not a hint — a missing file yields the *same* `Found no drivers!`, it does not fall back — so both branches failed at once and the fault looked system-wide. A comment in `007-run.sh` had described this stale path since 2026-08-27 without anyone acting on it. BUG-146 OPEN — while verifying the gate, found that `70-luminos-dgpu-access.rules` **has never fired at all**: `udevadm info /dev/nvidia0` says "Unknown device", because `nvidia-modprobe` creates the nodes with `mknod(2)` and no uevent is emitted. DECISION 25 rests on one layer (the `NVreg_DeviceFile*` driver param), not the two the docs claim — and that layer does not cover `nvidia_uvm`, which has no equivalent parameter, so `/dev/nvidia-uvm{,-tools}` come up **0666 root:root every boot**. Measured as *not* a bypass: `nvidiactl` and `nvidia0` stay tight, so `nvidia-smi` outside the gate still fails with "Insufficient Permissions". Left unfixed on purpose — gating UVM could take HIVE's CUDA models off the card and needs verifying against a running HIVE first. Also tracked the long-untracked `nvidia-egl-priority.hook` into the repo, and added `dgpu-exec-v2` to `install-dgpu-gate.sh`, which had only ever built v1. Earlier: BUG-142 FIXED — **the 007 black screen that came back after every restart was never a graphics bug; it was a leftover `explorer.exe /desktop` from the previous run.** One was found alive 17 minutes after its game had gone, still owning a fullscreen `Wine Desktop` window. The next launch makes a second window with the same title and class, KWin ends up holding `_NET_ACTIVE_WINDOW` on a window it keeps unmapped, and from then on every launch comes up with `_NET_WM_STATE_HIDDEN` — including launches made *after* the stale process is killed, because the wedged state is in KWin, not in Wine. The game itself was always fine: rendering, audio playing, GPU at 99%, into a window whose parent was `Unmapped`, so no frame ever reached the screen. That is why it looked intermittent for two sessions — whether a run is black depends on what the *previous* run left behind, which nothing logs. One-command discriminator: `xprop -id <win> _NET_WM_STATE`; the black case is the healthy list plus `_NET_WM_STATE_HIDDEN`. **KWin's scripting API lies about this** — it reports `minimized=false hidden=false` for a window the X server says is unmapped, and neither `w.minimized=false` nor `_NET_ACTIVE_WINDOW` recovers it. Fixed in `~/re/tools/007-run.sh` with `wineserver -k` before *and* after every attempt, plus a watchdog that spots `_NET_WM_STATE_HIDDEN` in ~40 s and restarts clean, then falls back to the 780M. `VKD3D_DEBUG=err` is now always on, which is how the rarer second mechanism was found: `vkCreateSwapchainKHR` returning `VK_ERROR_UNKNOWN` and being retried forever with no give-up path (rate later **measured at 13.1/s**; the "~100×/s" written here originally was a guess and was wrong by 8×). **Corrects BUG-138** — the virtual desktop size is not what decides it: 2880x1800 renders with the fullscreen rule off, 2560x1440 goes black with it on. Earlier: BUG-141 FIXED — **007 First Light would not launch because the phone's Dolphin chat was holding 5718 of the RTX 4050's 6141 MiB, and the number that actually decided it was not `nvidia-smi`'s free figure but the Vulkan heap `budget` the game is offered: 61.75 MiB against the ~2100 MiB it needs.** It died during device setup, three lines into its log. Commit `38e9c794` had predicted this outcome in its own message — *"`--ctx-size` is the first thing to lower if another GPU workload ever appears"* — which proves a prediction in a commit message is not a safeguard. Lowering the context could never have fixed it either: the 8B weights alone are ~4.6 GB. Fixed by finally building `MASTER_PLAN.md` Phase 6's GPU Manager as `scripts/luminos-gpu-yield`: the 007 launcher parks the model before Proton and `trap`s the restore on `EXIT INT TERM HUP`, so quitting the game gives the card back even on a crash or a Ctrl-C. **`restore` replays the saved `/proc/<pid>/cmdline` verbatim rather than calling `hive-start-model.sh <alias>`** — an alias would silently resurrect whatever that script hardcodes today, at the context it hardcodes today, which is not necessarily what was running. Measured: 73 → 5799 MiB in ~1 s, game took 3303 MiB at 99% GPU with 0 swapchain failures, Dolphin back and answering in ~3 s with byte-identical argv. Side bug: the launcher's low-VRAM warning had always gone to stdout, which does not exist when launched from the app menu — the one message that explained this was written to a closed fd every time; now a `notify-send` at the measured 2200 MiB floor. Earlier: BUG-111 FIXED — **the plugin death BUG-100 predicted came back on the first Hyprland bump, and the config error blamed the wrong file.** A login popup read `hypr-user.lua:397: unknown config key 'plugin.hyprexpo...'`, but nothing was wrong at line 397: Hyprland parses the config before plugins load, so an unloaded plugin turns its entire option namespace into unknown keys and sends you to edit Lua. hyprpm was built for 0.56.1 (`5c9377c1…`) while the compositor was 0.56.2 (`efb50993…`) — three days after DECISION 49 closed with "just run `hyprpm update` after upgrades". Now automated by `scripts/luminos-hyprpm-sync` from `hyprland.start`: compares build hash to running commit, rebuilds only on mismatch, then reads the loaded count back from `hyprctl` because `hyprpm update` prints a green `✔ Loaded` per plugin while loading none. **A pacman hook cannot do this** — hyprpm builds against the *running* compositor (so mid-transaction it would target the version being replaced) and hooks run as root (wrong `/var/cache/hyprpm/$USER`). Negative-tested all three failure paths before wiring it in. A brief error flash on the first login after an upgrade is now expected; only a popup that stays is a fault. BUG-105 FIXED — **the local LLM server was OOM-killed by long prompts, and every symptom pointed at the GPU.** `llama-cpp-python`'s server defaults `--logits_all` to True, keeping a logit vector for every prompt token: 19k tokens x 151,936 vocab x 4 B = ~11.5 GB of SYSTEM RAM on a 14 GB box. Short prompts worked, long ones died at ~30 s with a tidy `Shutting down` in the log — which is a lie; the real exit was rc=137/SIGKILL from the OOM killer, while VRAM sat at 4630 of 6141 MiB. Fixed with `--logits_all false`: peak RSS 8830 MB -> 817 MB, same request now answers in 13.4 s. Lesson: get the exit code before theorising, and don't assume the accelerator is at fault just because the workload runs on it. BUG-104 FIXED — **mempalace reported "success" and threw every memory away, for at least ten days.** `add_drawer` returned a drawer_id, the WAL logged the call, and the drawer did not exist; the WAL shows `"result": null` on every add since 2026-07-26, and content is redacted there so none of it is recoverable. Root cause is in ChromaDB 0.6.3, not MemPalace: the palace's per-segment `max_seq_id` watermark held a poisoned ~1.23e18 timestamp while `embeddings_queue` — an `INTEGER PRIMARY KEY` with no AUTOINCREMENT — had been emptied and restarted numbering at 1, so `_notify_one` skipped every record as "already consumed" and `upsert()` raised nothing. Repaired by setting each watermark to the queue's current max row id; zeroing it does NOT work, because `start = start or self._next_seq_id()` treats 0 as falsy. Verified by readback in a fresh process. **The running MCP server keeps the poisoned subscription in memory and `mempalace_reconnect` does not rebuild it** — restart the server or file through the library. BUG-103 FIXED — the dGPU never went back to sleep after you used it: all three GPU launchers wrote `on` to `power/control`, which *disables runtime PM for the device*, and nothing anywhere ever wrote `auto` back, so the card sat at 1.63 W / P8 / `active` with zero processes holding it. The fix was to **stop writing `on`** — with `control=auto` the driver takes a runtime-PM reference when a device node is opened, so the card wakes on demand and re-suspends by itself ~20 s after the last close. No release mechanism was needed; the one line meant to help was the only thing preventing sleep. Proven end to end: NVIDIA Chrome came up on `renderD128` with `glRenderer = ANGLE (NVIDIA, … RTX 4050 …)`, then the card returned to `suspended` on its own, and `luminos-verify` section 3 now passes all three checks. Two side-findings: `luminos-wine-launcher` had silently diverged from its installed copy (repo newer but missing both EGL exports — reconciled), and it **never calls the dGPU gate at all**, so Wine-on-NVIDIA is still denied like BUG-102. `luminos-gpu-launch` was also promoted from `dgpu-exec` to `dgpu-exec-v2` in the same pass. BUG-102 FIXED — picking "NVIDIA" in the Chrome GPU dialog silently gave you the AMD iGPU for a month, with a notification claiming otherwise. Three stacked causes: `chrome-luminos` never called the dGPU gate at all; the gate itself is defeated by **any launcher written in shell**, because setgid raises only the *effective* gid and bash resets it (fixed by `dgpu-exec-v2`, which `setresgid`s so the group is real); and Chrome is single-instance per profile, so the picker could never take effect while a window was open. Now proven on the card — `ANGLE (NVIDIA, Vulkan …RTX 4050…)`, 20 fds on `/dev/nvidia0`, listed in `nvidia-smi`. Two Chromes on two GPUs at once works, given separate `--user-data-dir`. BUG-101 FIXED — the SUPER launcher's app list "barely scrolled", on the touchpad only: Caelestia ships `input:touchpad:scroll_factor = 0.3`, and in a viewport only `maxShown` rows tall, 30% of a swipe travels less than one row. The mouse wheel uses the **separate** `input:scroll_factor`, already 1.0 — which is why the two devices behaved differently. Overridden to 1.0 in `hypr-vars.lua`; no QML touched. BUG-100 FIXED — every hyprpm plugin was dead because hyprpm was still building against an April compositor. BUG-094 FIXED **AND VERIFIED ON A REAL LOGIN** — Hyprland is now the live session, the pin took effect, the dGPU is `suspended`, and Claude Desktop runs on the AMD `renderD129`. Original report: the Hyprland session bounced straight back to SDDM: `AQ_DRM_DEVICES` is a COLON-separated list and the GPU pin was written as a PCI by-path, so one device path split into three nonexistent ones and the compositor aborted with "Found no gpus to use". Neither stock name works (by-path has colons, cardN is unstable), so a colon-free udev alias `/dev/dri/luminos-igpu` was created. BUG-093 FIXED — a user-site `packaging` copy shadowed the pacman one, so pacman said 26.2 while Python said 26.0 and every AUR python build failed; fixed with `PYTHONNOUSERSITE=1`, nothing removed. BUG-092 FIXED — SDDM greeter wallpaper pointed at a missing file *under `$HOME`*, which the `sddm` user could never read anyway; the resulting black login screen was misread as a Hyprland crash. **Note:** BUG-092 was first filed as BUG-091 and renumbered — 091 was already taken by the suspend bug below. BUG-091 FIXED — lid close and idle now suspend; the machine never had a suspend bug, only three layers of deliberate config, and the first fix landed in a PowerDevil config group nothing reads. BUG-087 FIXED — MCP tooling now reaches Claude Code, Claude Desktop and Antigravity; hooks moved to user scope because Cowork ignores project scope. BUG-085 FIXED — MCP tooling silently rotted; now pinned + verified by `luminos-verify --mcp`. BUG-086 CLOSED/WONTFIX — leaked OpenRouter key accepted by user as a dead account, no rotation. BUG-084 **FIXED** — DrKonqi gdb+debuginfod ate 7.4 GB and filled zram; now capped by `systemd/user/luminos-drkonqi.slice` at `MemoryMax=2G MemorySwapMax=0`. A **slice**, not a per-unit cap, because the 2026-07-24 incident was five concurrent `drkonqi-coredump-launcher@` instances, and a template's instance count is exactly the quantity a per-instance cap cannot bound; only the sum can be capped. `MemoryHigh` is **deliberately absent** — all four combinations were measured against a process faulting in 3 GiB of real anonymous pages, and the config originally shipped (`High=1500M`, swap allowed) was the *worst* of the four: with swap available `MemoryHigh` does not stop a runaway, it pushes the pages into zram, which is the original incident reproduced; with swap denied it prevents the kill from ever happening (anon pages cannot be reclaimed, so the cgroup spins in direct reclaim and the job neither dies nor finishes). Only `MemoryMax` + `MemorySwapMax=0` with no `MemoryHigh` produces a clean OOM-kill — verified at 2 GiB in 2 s, `result 'oom-kill'`, `status=9/KILL`. BUG-083 FIXED + measured. BUG-082 FIXED (pending live verify). BUG-080 — **root cause found, restart loop fixed, upgrade unverified.** It was never a Wine bug: MT5's terminal spawns `liveupdate\terminal64.exe /update` and exits 0, but the staged binary was byte-identical to the installed one, so the handoff could never make progress and systemd restarted it forever. Cleared both staging dirs (a second one, roaming/build 5984, was nearly missed because `find -maxdepth 8` does not reach a path 9 levels deep). Loop is definitively gone — 10m16s on a single start with no handoff line, against seconds-long restarts before. The upgrade to build 6140 could **not** be confirmed: the terminal stops on a modal Authorization dialog, so LiveUpdate never runs; credentials were not entered and trading units stayed masked throughout.)
...
- Status: **REBOOT TESTED 2026-08-29 — the gate LOST the boot race, exactly as feared. Now FIXED**
  by `config/udev/71-luminos-uvm-gate.rules`. World-open window at boot **2.2 s → 9-18 ms, measured.**
  Remaining sub-items (suspend/resume, driver upgrade, initramfs param) are still untested; the bug
  stays open for those.

#### The reboot result, 2026-08-29 (boot at 13:07:59)

The "fail loudly" instrumentation added earlier the same day is what caught it. It worked:

```
[    6.873686] nvidia-nvlink: Nvlink Core is being initialized       <- card binds, udev fires
[    9.089422] Starting Luminos dGPU gate: nvidia_uvm device nodes...
[    9.102202] luminos-uvm-gate: UNGATED NODE FOUND: /dev/nvidia-uvm was [root:root 666] ... Repaired,
[    9.102464]                     but it was open until this moment.
[    9.109766] luminos-uvm-gate: FAILED: the nodes are correct NOW, but this unit did not create them
[    9.110664] Failed to start Luminos dGPU gate: nvidia_uvm device nodes.
```

**Root cause of the race — found, not guessed.** `/usr/lib/udev/rules.d/60-nvidia.rules` runs
`nvidia-modprobe -c0 -u` on the nvidia PCI device's `add|bind` uevent, which happens at **t≈6.87 s**,
during initramfs-time module load (`mkinitcpio MODULES=(nvidia nvidia_modeset nvidia_uvm nvidia_drm)`).
`luminos-uvm-gate.service` cannot run until systemd reaches `multi-user.target` at **t=9.09 s**.
The gate was never going to win; it was **2.2 seconds** late, every boot. Item 1 of the original
list ("any of them could get there first") named `nvidia-persistenced`/`nvidia-powerd`/early KMS as
suspects — all three are innocent. It was udev running the driver's own helper.

#### THE BIG ONE: "winning the race" would have BROKEN THE CARD

Testing the alternative fix (pre-create the nodes first, so `nvidia-modprobe` skips them) turned up
something worse than the bug being fixed. `60-nvidia.rules` reads:
...
`TEST!="/dev/nvidia-uvm"` guards **both** `RUN+=` lines, not just the UVM one. So if
`/dev/nvidia-uvm` exists when that rule is evaluated, `nvidia-modprobe` does not run **at all** — and
`/dev/nvidia0` and `/dev/nvidiactl`, which the *first* RUN creates, are never created either.

**Measured 2026-08-29:** with the UVM nodes present and the main nodes deleted, an `add` trigger
brought back **neither**, and `nvidia-smi` failed with *"couldn't communicate with the NVIDIA driver"*.

This means the stated design goal of `luminos-uvm-gate.sh` — *"PRE-CREATE both nodes ourselves...
winning the race is enough"*, and the `created ... won the race` success message — **was the
catastrophic outcome, not the correct one.** The gate has only ever worked *because it lost*.
The mknod path is now guarded on `/dev/nvidia0` already existing, which is the proof that
`nvidia-modprobe` has already had its turn. Do not remove that guard, and do not write a udev rule
numbered **below** 60 that pre-creates these nodes.

#### The fix: `config/udev/71-luminos-uvm-gate.rules`

The gate script's header said *"the obvious fix — a udev rule — cannot work here"*. That claim is
true **only** of a rule keyed on the node (`KERNEL=="nvidia-uvm"`): `nvidia-modprobe` creates these
with `mknod(2)`, which emits no uevent, which is why `70-luminos-dgpu-access.rules` has never fired
for anything. But a rule keyed on the **PCI device** `add|bind` event works fine — that is a real
uevent, and it is the very event `60-nvidia.rules` itself uses. The over-broad wording steered the
design wrong for a day; it has been narrowed in place rather than deleted.

Ordering is the whole fix, and it was **measured, not assumed**: udev parses rule files in lexical
filename order and accumulates `RUN+=` commands in that order, executing them after all rules for the
event are parsed. Proof — with a marker script at 99 and both nodes deleted, the marker observed
them as `[root:root 666]`, i.e. `nvidia-modprobe` had already created them. Had we run first we
would have seen ABSENT.

| test | result |
|---|---|
| does a `RUN+=` on the nvidia PCI event fire at all? | **YES** — disproves the blanket "udev cannot work here" |
| does it run *after* `nvidia-modprobe`? | **YES** — marker saw `root:root 666`, not ABSENT |
| residual window, `/dev/nvidia-uvm` | **9-10 ms** (3 runs) — was 2.2 s |
| residual window, `/dev/nvidia-uvm-tools` | **18 ms** (3 runs) — was 2.2 s |
| 5x delete+trigger cycles, gated unaided? | **5/5 PASS** |
| backstop service after the change | `active`, exit 0, single `gated:` line |
| mknod guard with `/dev/nvidia0` absent | **REFUSED** to create, exit 1, nodes left absent |

The window is **not zero** and the rule file must not claim it is. Closing it completely requires
pre-creating the nodes, which is the card-killing path above.

#### Consequences for how this is read

- `luminos-uvm-gate.service` is now the **BACKSTOP**, not the fix. It still catches the case where
  the udev rule did not fire at all (removed, udev not reloaded, module loaded by another path).
- The healthy `systemctl status` is `active (exited)` with one `gated: ... root:dgpu 660` line and
  **no `created` line**. The old checklist below said *"want 'created' not 'already present'"* —
  **that is now exactly backwards** and is corrected in place.
- `--udev` mode was added to the script: in that context finding the nodes freshly created and
  world-open *is* the expected sequence, so the loud failure is suppressed there and only there.
- Two distinct bad outcomes are now tracked separately (`raced` vs `refused`). Folding them into one
  flag was written first and produced a flatly false message — the refusal path printed *"the nodes
  are correct NOW"* while both nodes were absent. Caught by the guard test.

#### Still open under this bug

Items 2 (initramfs param), 3 (suspend/resume), and 4 (driver upgrade) below remain **untested**.
The boot race — item 1 — is closed.

- Severity: Medium-High. If the boot gate loses its race, the failure is **silent** — the machine
  keeps working, DECISION 25 just quietly stops being true and nobody is told. *(Confirmed: this is
  exactly what happened, and only the deliberate fail-loudly instrumentation revealed it.)*
- Component: `config/udev/71-luminos-uvm-gate.rules` (**the fix**),
  `scripts/dgpu-gate/luminos-uvm-gate.sh`, `systemd/luminos-uvm-gate.service` (now backstop),
  `/etc/modprobe.d/luminos-dgpu-gate.conf`, `/usr/local/bin/dgpu-exec-v2`, `scripts/007-run.sh`.
- Verified working **in the current uptime only**, 2026-08-28/29: 007 First Light renders on the
  RTX 4050; HIVE's nexus loads 5718 MiB on the card through `dgpu-exec-v2`; `vulkaninfo` resolves
  the 4050 through the gate; `dgpu-exec-v2 --check` reports `access: OK / gate: OK`.
- **Why "it works now" is not evidence it will work after a reboot.** Every piece of this was
  applied, restarted or raced into place by hand during a session. Four separate things have to be
  true at boot instead, and each one is a different mechanism:
  1. **The UVM gate has to win a race it has never actually run.** `luminos-uvm-gate.service`
     pre-creates `/dev/nvidia-uvm{,-tools}` and that only works if it runs *before* anything calls
     `nvidia-modprobe`. It is ordered `Before=supergfxd.service` and `Before=display-manager.service`
     because supergfxd was the observed trigger — but that was observed *at runtime*, not at boot.
     A real boot has `nvidia-persistenced`, `nvidia-powerd`, initramfs-time module loading and the
     early KMS path, none of which were in the simulation. **Any of them could get there first**,
     and if one does the nodes come up `0666 root:root` exactly as before, with no error anywhere.
  2. **The driver param has to be in the initramfs.** `NVreg_DeviceFileGID` is only read at module
     load; `nvidia` is in `mkinitcpio MODULES=`, so it must be baked in. `mkinitcpio -P` was run —
     but a kernel or driver upgrade regenerates the initramfs, and nothing verifies the param
     survived.
  3. **Suspend/resume is a separate question from boot,** and the claim in
     `install-dgpu-gate.sh` that "nvidia-modprobe recreates /dev/nvidia* at 0666 on every wake" has
     never been re-tested since the UVM gate landed. If it is true, the gate needs a resume hook it
     does not have. If it is false, that comment should be deleted. Either way it is currently an
     unchecked assertion sitting in an install script.
  4. **A driver upgrade is a third distinct event.** The UVM device major is read from
     `/proc/devices` so a renumber is handled, but a package upgrade can restore
     `/usr/bin/nvidia-modprobe`, rewrite the vendor JSON that `dgpu-exec-v2` points at, and
     regenerate the initramfs — all at once, unattended, via a pacman hook.
     *Partly addressed 2026-08-29:* the setuid half is now defended by
     `config/pacman-hooks/luminos-nvidia-modprobe-setuid.hook`, which re-strips the bit
     PostTransaction on every `nvidia-utils` install/upgrade (BUG-146). **The hook itself has
     never run during a real upgrade** — it is on the same untested list as everything else here.
     The vendor JSON and initramfs halves are still undefended.
- **Pre-reboot hardening applied 2026-08-29.** The reboot itself is still untested — that is the
  whole bug — but the gate can no longer fail *quietly*, which was the part that made an untested
  reboot dangerous. Three candidates were considered and all three were tested, not just the one
  that worked:
  1. **`Before=nvidia-powerd.service` — proposed, tested, REJECTED as redundant.** It sounded like
     the obvious missing ordering edge. It is already guaranteed twice over: `nvidia-powerd`
     declares `After=supergfxd.service` and this unit is already `Before=supergfxd.service`, so the
     ordering holds transitively; and `nvidia-powerd` is `disabled` with an **empty `WantedBy`**, so
     it cannot start itself at boot at all — supergfxd starts it explicitly, which is by definition
     after us (`Did CommandArgs { inner: ["start", "nvidia-powerd.service"] }` in the boot journal).
     Established with `systemctl show -p After -p Before -p WantedBy`, not by reading docs. Adding
     the line would have cost nothing and implied a guarantee we did not need. The reasoning is
     recorded in the unit file so it is not re-proposed.
  2. **Fail loudly when the major is missing — APPLIED.** `luminos-uvm-gate.sh` used to `exit 0`
     when `nvidia-uvm` was absent from `/proc/devices`, logging "nothing to gate yet". Measured in a
     sandbox: that produced a green `active (exited)` while the gate had done nothing and
     `/dev/nvidia-uvm` stayed world-open. A gate that is not in place is a failure. It now exits 1.
     Safe at boot because **nothing `Requires=` this unit** — it is only *ordered* before supergfxd
     and display-manager, so a failure is loud without being fatal.
  3. **Report whether the race was WON — APPLIED, and this is the one that matters.** "Did we get
     there first?" is the single fact that decides whether the gate is sound, and it was previously
     invisible: winning and losing produced byte-identical output and exit 0 either way. The script
     now distinguishes three outcomes — *created* (won), *present and already correct* (benign,
     almost certainly a re-run in the same boot), and *present but WRONG* (**lost the race**: repair
     it, then **exit 1**). Losing is not harmless just because we repair it afterwards; the window
     where the node sat world-open was real, and on a slower boot the repair could land after a CUDA
     client has already opened it. The script cannot tell "lost the boot race" from "BUG-146 re-opened
...
- Proposed test, to be run when the user is present and can log back in (do NOT run unattended,
  ...
  ls -l /dev/nvidia*; systemctl is-enabled luminos-uvm-gate
  # reboot, then immediately, BEFORE launching anything:
  ls -l /dev/nvidia-uvm /dev/nvidia-uvm-tools     # want 0660 root:dgpu
  ls -l /etc/udev/rules.d/71-luminos-uvm-gate.rules   # the fix must be present at all
  systemctl status luminos-uvm-gate               # want active(exited), no start-limit
  journalctl -b -u luminos-uvm-gate               # want ONLY "gated: ..."; see note below
  dgpu-exec-v2 --check                            # want access: OK / gate: OK
  ls -l /usr/bin/nvidia-modprobe                  # want -rwxr-xr-x, NO 's' (BUG-146)
  systemctl --user show luminos-drkonqi.slice \
      -p MemoryMax -p MemorySwapMax               # want 2147483648 / 0 (BUG-084)
  # then, and only then, launch 007 and HIVE and re-check
  ```
  **The journal line matters more than the perms do — but the polarity is the OPPOSITE of what this
  checklist said before 2026-08-29.** It used to read *"want 'created' not 'already present'"*. That
  was wrong twice: `created` is unreachable (udev beats the unit by 2.2 s) and would break the card
  if it ever happened. With `71-luminos-uvm-gate.rules` in place the correct boot outcome is a single
  `gated: /dev/nvidia-uvm root:dgpu 660 ...` line and **nothing else**:
  - `gated:` alone, exit 0 → the udev rule did its job. **PASS.**
  - `UNGATED NODE FOUND ... Repaired` + exit 1 → the udev rule did **not** fire. Check the rule file
    exists and that `udevadm control --reload` ran. This is now the failure signal.
  - `created ...` → should be impossible at boot. If it appears, something suppressed
...
<!-- [CHANGE: claude-code | 2026-08-29] -->
- Status: **ROOT CAUSE FOUND AND FIXED**, 2026-08-29. 007 First Light now runs on the RTX 4050 with
  **zero** swapchain failures, where the same build produced 278 in 3 s and fell back to the 780M.
  **This closes BUG-142**, which was open for two days against the wrong component.
- Severity: **High** — it silently disabled NVIDIA presentation for every application on the
  machine, and it was a **regression we introduced ourselves** while fixing BUG-146.
- Component: `scripts/dgpu-gate/luminos-uvm-gate.sh` (the fix),
  `config/pacman-hooks/luminos-nvidia-modprobe-setuid.hook` (the cause, and it stays),
  `/usr/lib/udev/rules.d/60-nvidia.rules` (the gap).

**What happened.** BUG-146 stripped the setuid bit from `/usr/bin/nvidia-modprobe` and added a
pacman hook to keep it stripped. That was correct and is not being reverted. What nobody checked is
what else that binary was doing for us. `60-nvidia.rules` is four lines and asks for exactly two
things:
...
`-c0` makes `/dev/nvidia0`, `-u` makes the UVM pair. **Nothing ever passes `-m`.** So the only
thing that had ever created `/dev/nvidia-modeset` on this machine was some *application* invoking
the setuid helper on demand. Remove the setuid bit and that path disappears — the node stopped
being created, and no log anywhere mentioned it.

**Why it was invisible.** Everything that gets checked kept passing. `nvidia-smi` worked.
`dgpu-exec-v2 --check` printed `access: OK / gate: OK` — it listed `/dev/nvidia-modeset absent`
as a neutral line, not a fault. CUDA worked, so HIVE was fine. The dGPU was reachable, held the
right permissions, and drew power. Only *presentation* was broken, and nothing in the health
checks presents anything.

**The measurement.** Same binary, same boot, one node apart:

| `/dev/nvidia-modeset` | `vulkaninfo --summary` through the gate |
|---|---|
| absent | `vkGetPhysicalDeviceDisplayPropertiesKHR failed with ERROR_UNKNOWN`, aborts, no devices listed |
| present | `deviceName = NVIDIA GeForce RTX 4050 Laptop GPU`, clean |

**The control matters and was run:** the same `vulkaninfo` on AMD does **not** produce that error —
it enumerates the 780M normally. So this is NVIDIA-specific, not a `vulkaninfo` quirk and not a
headless/Wayland artefact. Restoring the node with `nvidia-modprobe -m` made the error vanish
immediately, and the node came back `root:dgpu 660` **unaided** — the `NVreg_DeviceFile*` driver
params do cover this one, unlike the UVM nodes.

**Live confirmation, not inference.** 007 launched on NVIDIA and stayed there: `007FirstLight.exe`
resident on the 4050 at 3613→4294 MiB, GPU 98-100%, 58-70 W, ~3 minutes, `.swapchain-failed` never
set, the AMD fallback never reached. User confirmed the game reported 6 GB VRAM and that the
desktop stayed responsive — which is the muxless split working as designed, the iGPU freed for the
compositor while the dGPU renders.

**Honest limit on the claim.** The call this fix was *proven* against is
`vkGetPhysicalDeviceDisplayPropertiesKHR`; the call 007 died on is `vkCreateSwapchainKHR`. They are
...
from both the udev rule and the backstop service, so the stripped setuid bit is irrelevant there —
that is exactly why the gate can do this when an ordinary application no longer can. `nvidia-modprobe
-m` is used rather than `mknod` because the minor is a driver convention (254 here), not something
`/proc/devices` reports. Absent-and-uncreatable is a **loud failure** with the BUG-142 symptom named
in the message, so the next person does not spend two days on vkd3d.

**Lessons that should outlive this entry.**
1. **Removing a privilege removes capabilities you did not enumerate.** The BUG-146 note correctly
   recorded that a non-setuid `nvidia-modprobe` can no longer load modules or create nodes on
   demand, and correctly listed the two nodes we had covered. It did not ask *which other nodes*
   that binary had been creating. The answer was one, and it cost two days.
2. **A green health check is only as wide as the things it checks.** `--check` saw the missing node
   and printed it without alarm. It should treat an absent `/dev/nvidia-modeset` as a fault.
3. **Run the control.** `vulkaninfo` failing on NVIDIA meant nothing until the same command was run
   on AMD and did not fail.
...
<!-- [CHANGE: claude-code | 2026-08-29] -->
- Status: **FIXED and measured**, 2026-08-29. Stop time **90 s → 0.17 s**, `result=success`, exit 0.
- Severity: Medium (no data loss — but it made every poweroff/reboot a 2-minute wait, and a
  SIGKILLed daemon gets no chance to flush state).
- Component: `scripts/hive-daemon.py` (`main()`, the `_shutdown` handler),
  `systemd/user/luminos-hive.service`.
- Symptom as reported by the user: *"when I try to turn off, some kind of timer comes and it takes
  around 2 min to turn off."* That is Plymouth's
  `A stop job is running for Luminos HIVE Daemon (1min 30s / 1min 30s)`.
- **Found by timing, not by guessing.** Diffing consecutive monotonic timestamps across the last
  eight boots' shutdown sequences: exactly **one** gap over 3 s existed in any complete shutdown
  record, and it was **90.0 s**, ended by:
  ```
  [40035.211260] systemd[851]: Stopping User Manager for UID 1000...
  [40125.271150] systemd[851]: luminos-hive.service: State 'stop-sigterm' timed out. Killing.
  [40125.271233] systemd[851]: luminos-hive.service: Killing process 946 (python3) with SIGKILL
  ```
  90.06 s, which is exactly `DefaultTimeoutStopUSec=1min 30s`. Everything else in the shutdown
  finished in milliseconds. (Only boot -3 has a complete record; the other boots' journals stop
  before poweroff, so they are consistent-but-inconclusive rather than confirming.)
- **Root cause — the handler existed and looked correct, which is why it survived so long.**
  `hive-daemon.py` *did* install a SIGTERM handler, so a grep for "does it handle SIGTERM" answers
  yes. The handler called `server.shutdown()`. That method **blocks until the `serve_forever()` loop
  acknowledges the request** — and Python delivers signals **on the main thread**, which is the very
  thread sitting inside `serve_forever()`. So the handler waited for a loop that could not run until
  the handler returned. Neither side could move, forever. The stdlib documents exactly this:
  `shutdown()` *"must be called while serve_forever() is running in a different thread otherwise it
  will deadlock."*
- **Reproduced in isolation before changing anything** (`/tmp/sigterm-test.py` — same
  `ThreadingHTTPServer`, same daemon background thread, SIGTERM to exit), and all four candidates
  measured rather than reasoned about:

  | approach | time to exit | verdict |
  |---|---|---|
  | `server.shutdown()` direct (the code as shipped) | **HUNG** >10 s, never exits | the bug |
  | `threading.Thread(target=server.shutdown).start()` | **0.51 s** | ← shipped |
  | `sys.exit(0)`, no `shutdown()` | 0.11 s | rejected |
  | `os._exit(0)` | 0.10 s | rejected |

  The two rejected options are faster precisely because they **skip** the orderly shutdown instead of
  performing it: in-flight requests are not drained and the listening socket is not closed. Draining
  is the entire purpose of `shutdown()`, so the fix keeps the call and moves it to a thread that is
  not the blocked one. The 0.51 s is one `serve_forever` poll interval, not a stall.
- Fix: `_shutdown` now dispatches `server.shutdown()` on a daemon thread and **does not** call
  `sys.exit(0)` — once dispatched, `serve_forever()` returns on its own and `main()` falls off the
  end normally. Insurance added alongside: `TimeoutStopSec=20` on the unit, so the *next* hang here
  costs 20 s instead of 90 s. That is a bound on the damage, **not** the fix.
- Verified live: the running old-code instance was stopped first and hung — `result=timeout`,
  `status=9/KILL`, killed at the new 20 s cap (it would have been 90 s). The fixed instance then
  stopped in 0.17 s with `result=success`, logging `Received SIGTERM — shutting down` for the first
  time. Three further stop/start cycles: 0.52 s / 0.16 s / 0.18 s, all `success`. HIVE serves
  normally afterwards (`/health` → HTTP 200).
- **Checked, and it is not a pattern:** the only two other Python daemons installing a SIGTERM
  handler (`src/daemon/main.py`, `src/classifier/router_daemon.py`) set a flag rather than calling
  `shutdown()`, which is correct. This bug was confined to `hive-daemon.py`.
- Side finding, same class as `007-run.sh` and `nvidia-egl-priority.hook`: `luminos-hive.service`
  **was not in git** — it existed only at `~/.config/systemd/user/`. Now tracked at
  `systemd/user/luminos-hive.service`.
- Side finding, NOT chased: boot -3 logged `Suppressed 55708 messages from user@1000.service` during
  the stall window. No suppressions on the current boot, so this looks transient rather than a second
  fault. Recorded so the number is not rediscovered as news.
- Verify: `time systemctl --user stop luminos-hive` should be well under 1 s with
  `Result=success`, never `timeout`.
- Date Found / Fixed: 2026-08-29

### BUG-145 — fixing the dGPU gate's privilege bug is what killed NVIDIA Vulkan
<!-- [CHANGE: claude-code | 2026-08-28] -->
- Status: **FIXED and verified**, 2026-08-28. `dgpu-exec-v2 vulkaninfo --summary` reports the
  RTX 4050, including through a `bash -c` wrapper, with the BUG-102 group fix still intact.
- Severity: High — and worse, **self-inflicted, and disguised as a regression in an unrelated fix**.
- Component: `scripts/dgpu-gate/dgpu-exec-v2.c`, `/etc/environment`.
- Symptom as reported: NVIDIA Vulkan appeared to be "dead system-wide". It was not. `dgpu-exec`
  (v1) showed the card the whole time; only `dgpu-exec-v2` failed:
  ```
  loader_scanned_icd_add: Could not get 'vkCreateInstance' via 'vk_icdGetInstanceProcAddr'
      for ICD libGLX_nvidia.so.0
  loader_icd_scan: Failed loading library associated with ICD JSON libGLX_nvidia.so.0. Ignoring
  vkCreateInstance: Found no drivers!   /   ERROR_INCOMPATIBLE_DRIVER
  ```
- **Root cause — `AT_SECURE`, and it runs backwards from intuition.**
  `/etc/environment` pins the whole machine to the Mesa EGL vendor (the BUG-046c / BUG-050 fix):
  `__EGL_VENDOR_LIBRARY_FILENAMES=/usr/share/glvnd/egl_vendor.d/50_mesa.json`.
  libglvnd reads that with `secure_getenv(3)`, which returns NULL when `AT_SECURE` is set. So:

  | gate | rgid/egid/sgid | AT_SECURE | Mesa pin | result |
  |---|---|---|---|---|
  | none | 1000/1000/1000 | 0 | obeyed | AMD only |
  | v1 (setgid only) | 1000/948/948 | **1** | **ignored** | AMD + RTX 4050 |
  | v2 (`setresgid`) | 948/948/948 | 0 | obeyed | AMD only, NVIDIA ICD discarded |

  NVIDIA's Vulkan ICD lives *inside* `libGLX_nvidia.so.0`, a glvnd-managed library — which is why
  an **EGL** variable silently kills **Vulkan**.
- **The counter-intuitive part: v1 never supported NVIDIA properly.** It worked by being privileged
  enough that the machine's own graphics policy did not apply to it. v2 is *more* correct, and being
  correct is exactly what exposed the pin. **Do not "fix" this by reverting to v1** — v1 is the
  version that silently hands the iGPU to every shell-launched app (BUG-102).
- The variable is present in the target's `environ` in **all three** cases. The kernel does not strip
  it. It is libglvnd that ignores it. Grepping the environment would never have shown this; the
  measurement that cracked it was a 12-line C probe printing `getauxval(AT_SECURE)` and `getresgid`.
- Fix: the gate now sets the NVIDIA vendor environment **itself**, after the credential change —
  saying out loud what v1 was getting by accident. A value already naming `nvidia` is left alone (so
  callers can choose, and re-entry is idempotent); a Mesa value or no value is replaced. If no NVIDIA
  vendor JSON exists it **unsets** the variable rather than pointing at a missing file, because unset
  means "scan the directory" and a dangling path means no EGL at all. `--keep-env` opts out.
- Also added `dgpu-exec-v2 --check`: prints identity, predicted target `AT_SECURE`, node perms, EGL
  vendor, ICD and power state, with a plain verdict. Every fact needed to spot this in seconds.

### BUG-144 — the AMD branch of the GPU picker handed apps zero Vulkan drivers
<!-- [CHANGE: claude-code | 2026-08-28] -->
- Status: **FIXED and verified**, 2026-08-28 — now resolves `AMD Radeon 780M Graphics (RADV PHOENIX)`.
- Severity: Medium, long-lived, and a genuine red herring in the BUG-145 hunt.
- Component: `scripts/luminos-gpu-launch:65`.
- Root cause: it exported `VK_ICD_FILENAMES=/usr/share/vulkan/icd.d/radeon_icd.x86_64.json`. Mesa on
  Arch ships a single-arch `radeon_icd.json`; the `.x86_64` name **has never existed on this box**.
- **The trap worth remembering: `VK_ICD_FILENAMES` is an override, not a hint.** Pointing it at a
  missing file does not fall back to the loader's default search path — it produces exactly the same
  `Found no drivers!` / `ERROR_INCOMPATIBLE_DRIVER` as BUG-145. So *both* branches of the GPU picker
  were broken, for two entirely unrelated reasons, which is why the failure looked system-wide.
- `~/re/tools/007-run.sh:88` had noted this stale path in a comment since 2026-08-27 without anyone
  acting on it. A comment describing a live bug elsewhere is not a fix; that comment is now retired.

### BUG-146 — the dGPU gate's udev "second layer" has never fired, and the UVM nodes are world-open
<!-- [CHANGE: claude-code | 2026-08-28] -->
<!-- [CHANGE: claude-code | 2026-08-29] root cause found; boot-time gate added; still open at runtime -->
<!-- [CHANGE: claude-code | 2026-08-29] FIXED — and Finding 3 below was WRONG about the cause. See Finding 4. -->
- Status: **FIXED**, 2026-08-29, pending only a reboot (tracked as BUG-147). Nodes hold
  `0660 root:dgpu` continuously, including across the daemon's own root control calls.
- **READ FINDING 4 BEFORE FINDING 3.** Finding 3 named the cause as "any NVIDIA client running with
  euid 0", and that is not right. It is close enough to sound right and it produced a fix plan that
  does not work. Finding 4 is the measured correction and is what the shipped fix is built on.
- Severity: Low-Medium. **Not a bypass** — see the measurement below — but it contradicts DECISION 25.
- Component: `config/udev/70-luminos-dgpu-access.rules`, `scripts/dgpu-gate/install-dgpu-gate.sh`,
  `scripts/dgpu-gate/luminos-uvm-gate.sh`, `systemd/luminos-uvm-gate.service`, `cmd/luminos-power/`.
- Finding 1 — the udev rule is decorative:
  ```
  $ udevadm info /dev/nvidia0     -> Unknown device "/dev/nvidia0": No such device
  $ udevadm info /dev/nvidia-uvm  -> Unknown device "/dev/nvidia-uvm": No such device
  ```
  The nodes are created by `nvidia-modprobe` with `mknod(2)`, not by the kernel device model, so no
  uevent is ever emitted and **no rule in that file has ever matched anything**. DECISION 25 is held
  up by exactly one layer — the `NVreg_DeviceFile*` driver param — not the two the docs claim.
- Finding 2 — that one layer does not cover UVM. `NVreg_DeviceFileGID` governs the `nvidia` module's
  own nodes; `nvidia_uvm` has no equivalent parameter (`/sys/module/nvidia_uvm/parameters/` has none).
  Measured after a clean boot:
  ```
  crw-rw---- root dgpu  /dev/nvidia0          <- gated
  crw-rw---- root dgpu  /dev/nvidiactl        <- gated
  crw-rw---- root dgpu  /dev/nvidia-modeset   <- gated
  crw-rw-rw- root root  /dev/nvidia-uvm       <- OPEN TO EVERYONE
  crw-rw-rw- root root  /dev/nvidia-uvm-tools <- OPEN TO EVERYONE
  ```
  `install-dgpu-gate.sh` step 7 *does* chgrp these — so this was intended — but that is a one-shot
  live fixup, and `nvidia-modprobe` recreates them at 0666 root:root on every boot.
- **Finding 3 — [2026-08-29] who actually re-opens them, caught with an audit watch.** The nodes are
  not merely created 0666 at boot; they are *reset* to 0666 repeatedly at runtime. Four wrong theories
  died first (nvidia-modprobe's create path, supergfxd, devtmpfs auto-creation, and a plain chmod that
  a syscall-path audit rule could see). A syscall rule with `-F path=` cannot catch `fchmod()` on an
  open fd, so the rule was replaced with an inode watch — `auditctl -w /dev/nvidia-uvm -p a` — which
  named the culprit immediately:
  ```
  comm="nvidia-smi" exe="/usr/bin/nvidia-smi" uid=0 euid=0 ppid=819
  proctitle: nvidia-smi --query-gpu=power.draw,temperature.gpu --format=csv,noheader,nounits
  syscall=90 (chmod -> 0666)   then   syscall=92 (chown -> root:root)
  ```
  PID 819 is `/usr/local/bin/luminos-power` (`luminos-power.service`, root). **Any NVIDIA client
  running with euid 0 re-applies the driver's device-file defaults**, and for the UVM nodes those
  defaults are hardcoded `0666 root:root`. It is our own thermal daemon that re-opens the hole.
  `nvidia-modprobe --help` confirms there is no permission option to point at a different mode.
  - Corollary: with `nvidia-modprobe` setuid (the distro default), an **unprivileged** `nvidia-smi`
    through the gate also resets them, because the setuid helper does the chmod as root.
  - Measured exposure window: at idle the nodes **stay** `0660 root:dgpu` (verified over 60 s) —
    `luminos-power` deliberately skips all dGPU sensing while the card is asleep, which is the
    BUG-103 power behaviour working as intended. The hole is therefore open only while the dGPU is
    awake, i.e. exactly while a GPU workload is running.
- **Exploitability, measured — the gate still holds.** CUDA and graphics both need `nvidiactl` and
  `nvidia0`, which stay tight. As `shawn` (not in `dgpu`): the UVM nodes open, `nvidiactl` and
  `nvidia0` are `Permission denied`, and `nvidia-smi` fails with `Failed to initialize NVML:
  Insufficient Permissions`. UVM alone gets you nothing usable. This is a defence-in-depth gap.
- **The CUDA risk that held this open is now disproven [2026-08-29].** The old worry was that gating
  UVM would take the HIVE models off the card. It does not. `open(2)` checks the *effective* gid, and
  HIVE's `llama-server` already runs through the gate. Measured with the UVM nodes at `0660 root:dgpu`:
  ```
  ungated          -> ggml_cuda_init: no CUDA-capable device is detected   (correctly blind)
  dgpu-exec   (v1) -> CUDA0: NVIDIA GeForce RTX 4050 Laptop GPU
  dgpu-exec-v2     -> CUDA0: NVIDIA GeForce RTX 4050 Laptop GPU
  full load via v2 -> Dolphin-8B, --n-gpu-layers 99, 4890 MiB resident, completion returned
  ```
- **What is fixed:** `systemd/luminos-uvm-gate.service` + `scripts/dgpu-gate/luminos-uvm-gate.sh`
  pre-create both nodes `0660 root:dgpu` at boot, ordered `After=systemd-modules-load` and
  `Before=supergfxd/display-manager`. This wins the creation race — `nvidia-modprobe` only mknods a
  node that is **absent**, so a correctly-owned node already in place is left alone. The script is
  also a repair path (`chown`/`chmod` an existing node) and reads the device major from
  `/proc/devices` rather than hardcoding it, since the kernel allocates it dynamically (237 today).
- **Finding 4 — [2026-08-29] Finding 3 blamed the wrong thing, and the plan it produced would have
  failed.** Finding 3 concluded "any NVIDIA client with euid 0 re-applies the defaults", which made
  the fix look like a single change: drop privileges in `luminos-power`. Before writing it, all four
  combinations were measured — repair the nodes, run one `nvidia-smi --query-gpu=power.draw`, stat
  the nodes:

  | caller | `nvidia-modprobe` setuid | gate holds? |
  |---|---|---|
  | non-root (`nobody:dgpu`) | yes | **NO** |
  | non-root (`nobody:dgpu`) | no  | **YES** |
  | root | yes | **NO** |
  | root | no  | **NO** |

  Two things are true at once, and each one alone re-opens the nodes:
  1. the **setuid-root helper** `/usr/bin/nvidia-modprobe`, which an unprivileged `nvidia-smi`
     invokes and which applies the driver's hardcoded `0666 root:root`; and
  2. **euid 0 in the caller itself**, which does the same thing without needing the helper.

  So dropping privileges alone does nothing (row 1) and dropping setuid alone does nothing (row 4).
  The fix is the **pair**. Finding 3's corollary had the setuid path right; its headline had the
  causality wrong, and the earlier note that dropping setuid "on its own buys nothing" was a correct
  statement dressed as a reason to discard it — it is one of two necessary halves.
- **What was fixed, in three parts** (all measured, all shipped 2026-08-29):
  1. `cmd/luminos-power/main.go` — new `nvidiaQuery()` runs every read-only `--query-gpu` poll with
     `SysProcAttr.Credential` set to uid 65534 / gid `dgpu`, mirroring what `dgpu-exec-v2` does.
     Three call sites converted (`readGPUStats`, `readGPUMaxGraphicsClock`, `initGPUTGP`).
  2. The setuid bit stripped from `/usr/bin/nvidia-modprobe`, kept stripped by
     `config/pacman-hooks/luminos-nvidia-modprobe-setuid.hook` — pacman restores the bit on every
     `nvidia-utils` upgrade, so a one-time `chmod u-s` silently reverts with nothing in any log.
  3. New `regateUVM()` / `nvidiaCtl()` — because four calls **cannot** drop root: `-pm`, `-pl`,
     `--lock-gpu-clocks`, `--reset-gpu-clocks` change hardware state. Measured with setuid already
     stripped, a single root `nvidia-smi -pl 55` still reset both nodes. Rather than pretend that
     hole is not there, every root control call now goes through `nvidiaCtl()`, which re-applies
     `root:dgpu 0660` immediately after. Exposure drops from "continuously, while the card is awake"
     to "for the duration of one `nvidia-smi` invocation, a handful of times per session".
- Verified end to end after the fix: 40 s soak with the daemon polling — nodes held `root:dgpu 660`
  throughout, **including across the startup `GPU TGP → 55W`**; `dgpu-exec-v2 nvidia-smi -L` shows the
  RTX 4050; `dgpu-exec-v2 sh -c 'exec 3<>/dev/nvidia-uvm'` succeeds while the same command ungated is
  `Permission denied`; ungated `nvidia-smi` still `Failed to initialize NVML: Insufficient Permissions`.
- Side finding: `initGPUTGP`'s `--query-gpu=power.limit` returns `[N/A]` on this card **even as root**,
  so it has always fallen through to `setGPUTGP(55)`. Pre-existing, not a regression — but it means
  the "current limit" log line has never once been printed.
- Cost of part 2, stated plainly: `nvidia-modprobe` exists so an unprivileged process can load the
  modules and have nodes created on demand. Without setuid it can do neither. That is only safe
  because Luminos already does both up front — `/etc/modules-load.d/nvidia.conf` loads
  `nvidia`/`nvidia_uvm` at boot, and the nodes are created by the module and by
  `luminos-uvm-gate.service`. **If either of those is ever removed, the hook must go with it.**
- **Two mechanisms evaluated and rejected**, recorded so they are not re-tried:
  1. *A paired `.path` unit.* Tried and removed the same day. `PathExists=` stays satisfied while the
     path exists, and this service **creates** the path rather than consuming it, so it retriggered
     itself ~4x/second until systemd hit the start limit and left the service `failed`. It was also
     unnecessary: device nodes live in devtmpfs and survive `rmmod nvidia_uvm` with permissions
     intact (verified), so once correct at boot they stay correct for the session.
  2. *Dropping the setuid bit from `/usr/bin/nvidia-modprobe`.* **This entry was wrong and is kept
     only as a record of the mistake.** It was rejected on 2026-08-28 with "on its own it buys
     nothing" — true, but the same is true of the privilege drop it was rejected in favour of. Both
     are necessary; neither is sufficient. See Finding 4. It is now shipped, with the pacman hook
     that this entry correctly predicted would be needed.
  3. *`NVreg_ModifyDeviceFiles=0`* is the only known complete fix, but it cannot be tested without a
     reboot and it would stop the driver applying `NVreg_DeviceFileGID` to `nvidia0`/`nvidiactl` too —
     trading a driver-enforced guarantee on the nodes that actually matter for a shell script. Not
     attempted.
- Detection is wired up now: `dgpu-exec-v2 --check` marks any node that is world-accessible or not
  `dgpu`-owned with `<- LEAK`, and reports `gate: LEAKING` separately from `access: OK`. The two are
  deliberately not merged into one number — a leak does not cause an iGPU fallback, and reporting
  them together would send the next reader hunting for the wrong fault.

### BUG-142 — the 007 black screen that came back after every restart was a leftover `explorer.exe`
<!-- [CHANGE: claude-code | 2026-08-27] -->
- Status: **FIXED and verified**, 2026-08-27. Six consecutive launches render, including three
  back-to-back quit/relaunch cycles and one launch made with a stale Wine session deliberately
  planted first.
- <!-- [CHANGE: claude-code | 2026-08-29] --> **SECOND MECHANISM ALSO CLOSED, 2026-08-29 — see
  BUG-149.** The `vkCreateSwapchainKHR` → `VK_ERROR_UNKNOWN` storm documented further down this
  entry was a **separate bug with a completely different cause**, and it was **ours**: stripping the
  setuid bit from `nvidia-modprobe` (the BUG-146 fix) removed the only thing creating
  `/dev/nvidia-modeset`, because `60-nvidia.rules` never passes `-m`. With the node restored, 007
  ran on the RTX 4050 for minutes at 98-100% GPU with **zero** swapchain failures and never reached
...
- Symptom as reported: *"the game's video is visible its black every time you fix it i can play i
  restart and it stops working find a permanent cure."* The word doing the work is **restart**.
- **Root cause, measured — it is not a graphics bug, it is a leftover process.**
  `explorer.exe /desktop` does not reliably exit when the game does. One was observed **alive 17
  minutes after its game was gone** (pid 127547, started 17:19, still there at 17:36), still owning
  a fullscreen X window titled `Wine Desktop`, class `steam_proton`. The next launch creates a
  **second** window with the same title and class. KWin then holds `_NET_ACTIVE_WINDOW` on a window
  it keeps unmapped, and from that point every launch comes up hidden — **including launches made
  after the stale process has been killed**, because the wedged state lives in KWin, not in Wine.
- **What the failure actually looks like at the X level** (this is the part that took two sessions):
  ```
  0x4600068  Unviewable  2880x1800            <- the Vulkan surface
  0x5200001  Unviewable  2880x1800  007 First Light
  0x1e00007  Unmapped    2880x1800  Wine Desktop   <- the parent. nothing below it can be seen
  0x359      Viewable    2880x1800            <- root
  ```
  The game is fine. It renders, audio plays, the GPU is at 99%. Its window is `Unviewable` because
  its **parent** is `Unmapped`, so not one frame ever reaches the screen.
- **The one-command discriminator.** `xprop -id <win> _NET_WM_STATE`:
  - rendering: `MAXIMIZED_VERT, MAXIMIZED_HORZ, FULLSCREEN, FOCUSED`
  - black: **the same list plus `_NET_WM_STATE_HIDDEN`**

  KWin's own scripting API disagrees with this — it reports `minimized=false fullscreen=true
  active=true hidden=false` for a window the X server says is unmapped, and setting
  `w.minimized=false` or sending `_NET_ACTIVE_WINDOW` does **not** recover it. Do not trust the
  KWin script view; trust `xprop` and `XGetWindowAttributes`.
  (`xwininfo`, `xdotool` and `wmctrl` are **not installed** on this box. `xprop` is.)
- **Why it looked intermittent for two sessions.** Whether a run is black depends on whether the
  *previous* run left `explorer.exe` behind — something neither the log, nor the run itself, nor
  any tool records. That is also why every "fix" appeared to work: each one happened to involve a
  teardown or a long enough gap.
- **The fix, in `007-run.sh`:**
  ...
  5. `VKD3D_DEBUG=err` is now **always on**, so the other way this can go black — a swapchain that
     is never created — is in the log instead of invisible (see next point).
- **The second, rarer mechanism, also proven today.** `vkCreateSwapchainKHR` returns
  `VK_ERROR_UNKNOWN` (`vr -13`) inside
  `dxgi_vk_swap_chain_recreate_swapchain_in_present_task`, and vkd3d retries it forever — there is
  no give-up path — so the first failure is fatal and the run only *looks* alive. The retry rate was
  **measured on 2026-08-29 at 13.1 failures/second**. This file previously said "~100×/second";
  that was a guess and it was wrong by 8×. It matters: a detector tuned to 100/s would never have
  fired on the real thing. `scripts/007-run.sh` uses the measured number. vkd3d ships with its log off, which is why nothing ever reached
  `~/re/007/last-run.log`. The watchdog now catches this too.
- **Ruled out by direct experiment. Do not spend another session on these:** NVIDIA
  Streamline/DLSS/NVAPI (`PROTON_DISABLE_NVAPI=1` moved VRAM 3304→2669 MiB, screen stayed black);
  the game's saved settings and profile (whole `remote/` removed → worked; restored → still
  worked); the `vkd3d-proton.cache`; driver/kernel-module mismatch (they match; no upgrades since
  Aug 24); winewayland vs winex11 (`winex11.drv` confirmed loaded); **the virtual desktop size**
  (2880x1800 and 2560x1440 both render, and both have gone black); **the BUG-138 KWin fullscreen
  rule** — black *with* it and *without* it, rendering *with* it and *without* it, so it is not the
  variable; suspend/resume; `vkcube --wsi xcb` at 2880x1800 works fine on NVIDIA.
- **Correction to BUG-138.** Its stated cause — "asking for a virtual desktop larger than KWin will
  give you" — did not survive testing. 2880x1800 renders correctly *with* the fullscreen rule
...
- Status: **FIXED and verified end to end on the card**, 2026-08-25. The card is now arbitrated:
  launching the game parks the HIVE model, quitting the game puts it back, and neither the user nor
  the launcher has to think about it.
- Severity: Medium (the game is unlaunchable; nothing is broken or corrupted)
- Component: `scripts/hive-start-model.sh` (`--ctx-size`) vs `~/re/tools/007-run.sh`. **Neither file
  is at fault.** They are two correct programs asking for the same 6 GB.
- Symptom as reported: *"the game 007 is not launching right now don't know the problem, I think it's
  the VRAM things."* He was right.
- **Root cause, measured.** `llama-server` holds **5718 of 6141 MiB**, leaving **73 MiB**.
  `vulkaninfo` on the NVIDIA ICD reports the device-local heap as `size 6.00 GiB, budget 61.75 MiB`
  — 61.75 MiB is the number the game is actually told it may allocate. 007 needs ~2074 MiB. It dies
  during device setup, three lines into `~/re/007/last-run.log`, right after
  `wineserver: NTSync up and running!` and before any renderer output.
- **This was predicted in writing by the commit that caused it.** `38e9c794` raised Dolphin's context
  12288 → 16384 to reclaim the retired forex bot's reservation, and its own message says: *"That fits
  nothing else, so `--ctx-size` is the first thing to lower if another GPU workload ever appears."*
  The game was the workload that appeared. **A prediction in a commit message is not a safeguard** —
  nothing enforces it, and the cost landed five days later on someone trying to play a game.
- **Lowering `--ctx-size` does not solve it.** Dolphin-8B Q4_K_M weights alone are ~4.6 GB; dropping
  16384 → 4096 frees roughly 0.85 GB of q8_0 KV cache and still leaves under 1.3 GB. The 8B model and
  the game do not both fit in 6 GB at any context. Do not waste a session tuning the knob.
- **Do NOT re-chase BUG-137 or BUG-138.** Both fixes are intact and were verified still in place:
  the KWin rule `Luminos: Wine virtual desktop is fullscreen (BUG-138)` is present in
  `~/.config/kwinrulesrc`, the session is Caelestia-on-Plasma with `kwin_wayland` + `plasmashell`
  running, and the game exe, the `GE-Proton10-34-vkd3d301` build and its prefix all exist. This is
  not a shader-compiler segfault (`assert=9 gpu=0%`) and not a swapchain extent mismatch
  (`assert=0 gpu=100%`). It is an allocation failure before either stage is reached.
- **The launcher already warns about exactly this and nobody sees it.** `007-run.sh:205-210` prints
  the free VRAM and names the holders when free < 4500 MiB — on **stdout**. Launched from the app
  menu or Lutris there is no terminal, so the one message that would have answered this in two
  seconds went to a closed file descriptor. It should be a `notify-send`, and below ~2200 MiB it
  should arguably refuse to start rather than hand the user a silent failure.
- **Diagnostic that settles this class of bug in one command:**
  `dgpu-exec-v2 nvidia-smi --query-compute-apps=pid,process_name,used_memory --format=csv`.
  For what the *application* will be told (which is the number that decides success), read
  `budget` under `memoryHeaps[0]` in `vulkaninfo` — `nvidia-smi`'s free figure and Vulkan's heap
  budget are not the same thing and the game only ever sees the latter.
- Historical note on the same fault line: on 2026-08-20 the game ran with only 2214 MiB free while
  `jobhunt-llm` held 3576 MiB, and its in-game VRAM meter went red. That was cosmetic. This is not
  — the difference between 2214 MiB and 73 MiB is the difference between an ugly readout and no
  game at all.
- **The real fix already had a design and had never been built.** `MASTER_PLAN.md` Phase 6 "GPU
  Manager" specifies exactly this: *"Unloads HIVE models instantly when game launches. Reloads from
  RAM cache when game closes."* Until something arbitrated the card, every new GPU consumer would
  silently evict a previous one and the failure would always surface as an unrelated app
  mysteriously breaking. So it was built — see below.

#### The fix — `scripts/luminos-gpu-yield` (installed as `/usr/local/bin/luminos-gpu-yield`)

- A small arbiter with four verbs: `status`, `yield`, `restore`, and `run -- CMD…`. `007-run.sh`'s
  NVIDIA branch calls `yield` before Proton and `trap`s `restore` on `EXIT INT TERM HUP`.
  `007 --keep-model` opts out. The `--igpu` branch is deliberately untouched — the 780M draws on
  shared system RAM and has never contended for anything.
- **The one rule the whole design is shaped by: never fail to give the card back.** A model that
  does not come back is a worse failure than a game that does not start, so `restore` runs from a
  trap, and the launcher no longer `exec`s Proton — it has to outlive the game to fire that trap.
- **`restore` replays the saved `/proc/<pid>/cmdline` verbatim; it does NOT call
  `hive-start-model.sh <alias>`.** An alias restores whatever that script hardcodes *today*, at the
  context it hardcodes today. A session that had launched a different model, or the same model at a
  different `--ctx-size`, would silently come back as something else — and the user would never be
  told. argv is the only honest record of what was running. It is saved NUL-separated and read with
  `mapfile -t -d ''` so arguments containing spaces survive the round trip.
- **`yield` saves argv BEFORE it kills anything, and refuses to kill if the save fails.** A model
  that cannot be described is a model that cannot be restored.
- **`restore` is idempotent and never starts something that was not there.** Nothing parked means
  nothing starts. Booting a 5.7 GB model at the end of a game played on a quiet machine would be a
...
- **A dead process is not freed VRAM.** The pid leaving the process table and the driver reclaiming
  its allocations are two different events, so `yield` polls the card until the memory is actually
  back instead of sleeping a magic number.
- **`pkill -f llama-server` would match the arbiter itself** — the string is in its own argv and in
  its state file path. Always `-x` on the exact comm. Same trap on the game side: comm truncates at
  15 characters, so `007FirstLight.exe` never matches and `007FirstLight.e` does.
- **Measured, both directions:**
  - yield: 73 MiB → **5799 MiB free in ~1 s**.
  - game: **3303 MiB** allocated on the RTX 4050, **99% GPU util, 54.56 W, 0 swapchain failures** —
    against the 2074 MiB it was starved down to on 2026-08-20.
  - **Looked at it, did not infer it:** the screenshot shows the full-bleed 007 First Light title
    screen, no titlebar, no bar. BUG-138's fix is intact under the new launcher.
  - restore: Dolphin back and answering `/health` **~3 s** after the game exited, argv **byte-identical**
    to what was parked (`cmp` clean), same model, same `--ctx-size 16384`, and a live
    `/v1/chat/completions` round trip returned `back online`. The phone chat is live again.
- **Side bug fixed in the same pass:** the launcher's low-VRAM warning had always gone to **stdout
  only**, which does not exist when launched from the app menu or Lutris — so the one message that
  explained this failure was written to a closed fd every single time. It is now a `notify-send`,
  and the threshold moved from 4500 MiB (a guess) to 2200 MiB (the game's measured floor).
- **Checked that nothing else would fight the arbiter before wiring it in:** `luminos-hive.service`
  inactive, `hive-idle-watchdog.service` inactive and in any case only ever kills, never starts, and
  `luminos-mobile-chat.py` is a pure proxy that never auto-starts the model.
...
- Status: **FIXED** (2026-08-29, measured — cap applied and verified to OOM-kill a real runaway. Incident originally cleared by hand 2026-07-24.) This is **insurance, not the root fix**: `kscreen-doctor` still SIGABRTs reliably on this box and still arms a launcher every time. The cap bounds the damage; it does not stop the crashes.
...
- Fix applied 2026-08-29 — two files, both in the repo and installed to `~/.config/systemd/user/`:
  - `systemd/user/luminos-drkonqi.slice` — `MemoryAccounting=yes`, `MemoryMax=2G`, `MemorySwapMax=0`.
  - `systemd/user/drkonqi-coredump-launcher@.service.d/luminos-memcap.conf` — `Slice=luminos-drkonqi.slice` (the shipped unit sets `Slice=app.slice`, so this must **override**, not add).
- **Why a slice and not `MemoryMax=` on the unit, which is what this entry originally recommended.** The obvious per-instance cap does not fix what was actually measured. The incident was not one greedy gdb, it was **five launchers at once**. `drkonqi-coredump-launcher@` is a *template*, so the instance count is set by how many things crashed — precisely the quantity a per-instance cap cannot bound. A 2G per-instance cap would have permitted 10G on a 14 GiB box: a stall, not a fix. A slice caps the **sum**; five concurrent backtraces now share 2G and OOM-kill each other inside that cgroup.
- **Why there is no `MemoryHigh=`, and why the first version shipped was the worst possible config.** The slice originally paired `MemoryMax=2G` with `MemoryHigh=1500M`, on the usual reasoning that a soft throttle is gentler than a kill. Tested against a process faulting in 3 GiB of real anonymous pages, all four combinations, 2026-08-29:

  | MemoryHigh | MemorySwapMax | outcome |
  |---|---|---|
  | 1500M | (allowed) | **BREACHED** — allocated the full 3 GiB in 10 s |
  | off | (allowed) | **BREACHED** — allocated the full 3 GiB in 7 s |
  | 1500M | 0 | **LIVELOCK** — throttled below 1.6 GiB, still running at 45 s, never killed, never finished |
  | off | 0 | **OOM-KILLED at 2 GiB in 2 s** ← the only one that works |

  With swap available `MemoryHigh` does not stop a runaway, it pushes the pages into zram — which *is* the 2026-07-24 incident, reproduced. With swap denied it prevents the kill from ever happening: anonymous pages cannot be reclaimed, so the cgroup spins in direct reclaim making near-zero progress and the job neither dies nor completes. A backtrace hung forever is not better than one that failed. `MemorySwapMax=0` is therefore **load-bearing, not a nicety** — without it `MemoryMax` is unenforceable in practice.
- Measured result on the shipped on-disk config: `result 'oom-kill'`, `status=9/KILL`, `Memory peak: 2G (swap: 0B)`. `systemd-analyze --user verify` rc=0.
- **Rejected on purpose: `Environment=DEBUGINFOD_URLS=`.** It would reliably prevent the blowup — debuginfod pulling full Qt/KDE symbol sets is what makes gdb grow — but it also makes every backtrace useless for reporting upstream, which is the only reason DrKonqi exists. The cap keeps the feature and bounds the damage.
- What this costs: a crash report for a very large core may now be truncated or fail. The core file itself is untouched in `~/.cache/drkonqi/cores/` and in `coredumpctl`, so a backtrace can always be produced afterwards with `coredumpctl gdb <pid>`. Nothing is lost that was not already on disk.
- **Four test-harness errors preceded the correct answer, each producing a confident wrong result** — recorded because each is easy to repeat: (1) looked for the cgroup at `.../user@1000.service/luminos-drkonqi.slice`, but the dash makes the slice nest under `luminos.slice`; (2) `bytearray(n)` goes through `calloc()` and glibc skips the memset on fresh mmap'd pages because the kernel already zeroed them, so 3 GiB was "allocated" while nothing was resident and nothing charged — the allocator must write real bytes to every page; (3) set `MemoryHigh`/`MemoryMax` on the **service**, which isolates nothing because the parent slice's limits still apply to every child; (4) the unique marker string appeared inside systemd's own `Started …python3 -c "…"` log line, which echoes the source verbatim, so the grep matched on every run.
...
- Verify: `cat /proc/pressure/memory` — `full avg10` should sit at ~0. `systemctl --user list-units --all 'drkonqi-coredump-launcher@*'` should show nothing running. The cap itself: `systemctl --user show drkonqi-coredump-launcher@1.service -p Slice` should say `luminos-drkonqi.slice`, and `systemctl --user show luminos-drkonqi.slice -p MemoryMax -p MemorySwapMax -p MemoryHigh` should read `2147483648 / 0 / infinity`.
- Date Found: 2026-07-24 — Fixed: 2026-08-29
...
- Status: **PARTIALLY FIXED** (2026-08-29 — real root cause found and the restart loop is definitively gone; the build upgrade could not be confirmed because the terminal stops on a login prompt). **The title of this bug is wrong and is kept only so the old references still resolve: Wine was never the cause.** Trading units remain masked; nothing was traded, no credentials were entered.
...
- Root Cause (**CONFIRMED 2026-08-29 — it is MT5's own LiveUpdate, not Wine**): MetaTrader stages an update into a `liveupdate\` subdirectory, spawns `liveupdate\terminal64.exe /update` to perform the swap, and **exits 0 itself** so the staged copy can replace the running binary. That exit-0-after-3s is the "clean exit" this bug was filed on, and it is by design. The loop existed because the staged binary was **byte-identical to the installed one** (same md5), so the handoff could never make progress: swap, restart, discover an update is still pending, stage the same bytes, hand off, exit 0, systemd restarts — forever. Wine 11.13 is exonerated; the version bump was coincidental timing.
- Fix applied: cleared the stale staging directories so the terminal has nothing to hand off to. There were **two**, not one — the second lived under the roaming profile (build 5984) and was nearly missed because `find ~/.wine -maxdepth 8 -name liveupdate` returns nothing for a path that is nine levels deep.
- Measured: the terminal now stays up **10m16s on a single start with no handoff line in the log**, against seconds-long restart cycles before. `terminal64.exe` md5 unchanged, confirming no swap was attempted.
- **NOT verified — the remaining work.** The upgrade to build 6140 never ran, because the terminal comes up on a modal **Authorization** dialog (login 107166028, MetaQuotes-Demo, "Save password" unchecked) and LiveUpdate does not run until it is logged in. Credentials were deliberately not entered. Until someone logs in once with "Save password" ticked, the terminal will keep sitting on that dialog and port 18812 will keep staying closed — which will look exactly like this bug even though the loop is fixed.
- Diagnostic traps hit here, worth remembering: `ss -tnp` reports almost nothing in this sandbox (network visibility is restricted), so "0 sockets" is not evidence of "not connected" — count fds in `/proc/<pid>/fd` instead, or take a screen capture. `import`/`xdotool`/`xwininfo`/`wmctrl`/`xwd` are all absent; `ffmpeg -f x11grab -video_size 1024x768 -i :99 -frames:v 1` works. And the MT5 config directory is `Config` with a capital C — a lowercase `ls` says "No such file or directory" and makes it look like there is no saved account.
- Verify: `(exec 3<>/dev/tcp/127.0.0.1/18812)` should succeed (port OPEN) and `systemctl --user status forex-bot` should reach `active (running)`. Before that can happen, MT5 must be logged in once interactively.
- Date Found: 2026-07-21 — Root cause found / loop fixed: 2026-08-29
