...
- `scripts/luminos-caelestia-kwin` — The session script; `kwin_wayland` runs it and exits when it returns, so this *is* the session. Deliberately **not** `startplasma-wayland` with plasmashell suppressed: that needs a `LUMINOS_SHELL` env var, and `Linger=yes` means the systemd user manager outlives logout and would carry it into a later plain-Plasma login — a Plasma session with no shell and no explanation. Starts polkit-kde, xdg-desktop-portal-kde and the shell; **not** powerdevil (logind owns the lid; powerdevil would fight Caelestia over the brightness keys) so there is no idle screen-off yet. Also **not** `kglobalacceld` — on Plasma 6 `kwin_wayland` owns `org.kde.kglobalaccel` itself and a standalone daemon exits instantly with an empty log. Shell restarts 3× then falls back to a kitty window showing the log (BUG-092: a black screen is not evidence of a crash). Logs in `~/.local/state/luminos/caelestia-kwin/{session,shell,sycoca,polkit,portal-kde}.log`. [CHANGE: claude-code | 2026-08-09]
- `scripts/luminos-caelestia-kwin-overlay` — Python. `SRC=/etc/xdg/quickshell/caelestia` → `DST=~/.config/quickshell/caelestia-kwin`. Symlink farm + two generated files, so upstream stays authoritative across package upgrades. Patched: `services/ShellState.qml` (`forActive`/`componentsForActive` fall back to `instances[0]` **after** the match loop — see BUG-113 for why "before" is not enough) and `services/Brightness.qml` (`getMonitor("active")` falls back to `monitors[0]`). `patched_text()` asserts each anchor appears exactly once and `sys.exit`s otherwise. `--check` is safe to run any time and is wired into the session installer's check. [CHANGE: claude-code | 2026-08-09]
