...
- `scripts/luminos-caelestia-kwin-overlay` — Python. `SRC=/etc/xdg/quickshell/caelestia` → `DST=~/.config/quickshell/caelestia-kwin`. **279 symlinks + 4 generated files**, so upstream stays authoritative across package upgrades. `patched_text()` asserts each anchor appears exactly once and `sys.exit`s otherwise, so an upstream change fails the build loudly instead of shipping a stale copy. `--check` is safe to run any time and is wired into the session installer's check.

  The four patched files, and the one bug they all are:
