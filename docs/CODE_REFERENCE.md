...
- `config/udev/71-luminos-uvm-gate.rules` — **The** fix for the DECISION 25 boot race (BUG-147); deployed to `/etc/udev/rules.d/`. Runs `luminos-uvm-gate --udev` on `ACTION=="add|bind"` for the NVIDIA PCI **device** — the same uevent `60-nvidia.rules` uses to invoke `nvidia-modprobe`, which creates `/dev/nvidia-uvm{,-tools}` hardcoded `0666 root:root`. Filename must sort **after** 60 (udev accumulates `RUN+=` in lexical file order), so nvidia-modprobe runs first and we repair second: world-open window 2.2 s → **9-18 ms, measured over 3 runs; not zero**. Keyed on the PCI device, **not** the node — `mknod(2)` emits no uevent, which is why the node-keyed `config/udev/70-luminos-dgpu-access.rules` has never fired for anything. **Never renumber this below 60 and never make it pre-create the nodes**: `60-nvidia.rules` guards *both* its `RUN+=` lines with `TEST!="/dev/nvidia-uvm"`, so an existing UVM node suppresses nvidia-modprobe entirely and `/dev/nvidia0`+`/dev/nvidiactl` are never created (measured: `nvidia-smi` "couldn't communicate with the NVIDIA driver"). Kill switch: `rm` the file + `udevadm control --reload`; the backstop service then repairs at ~9 s as before. [CHANGE: claude-code | 2026-08-29]
- `scripts/dgpu-gate/luminos-uvm-gate.sh` — Brings `/dev/nvidia-uvm{,-tools}` under DECISION 25 (BUG-146); deployed to `/usr/local/bin/luminos-uvm-gate`, run both from the udev rule above and from `systemd/luminos-uvm-gate.service` (now the **backstop**, not the primary mechanism). Reads the nvidia-uvm major from `/proc/devices` (dynamic — 237 on 2026-08-29, do not hardcode), then chowns `root:dgpu` / chmods `0660`. Exits **non-zero** on: group missing, major missing (module not loaded — a gate that is not in place must say so, it used to `exit 0` here and go green while doing nothing), a node found ungated and repaired (`raced`), or a node absent that it refused to create (`refused`). Those last two are deliberately separate flags with separate messages — folding them into one printed "the nodes are correct NOW" while both nodes were absent. `--udev` suppresses only the `raced` failure, because riding nvidia-modprobe's own uevent and finding the nodes freshly world-open **is** the expected sequence there. The mknod path is guarded on `/dev/nvidia0` existing, which is the proof nvidia-modprobe has already had its turn. **Also creates `/dev/nvidia-modeset` (BUG-149)** — despite the file's name, because it is already the thing running as root on the nvidia PCI uevent with the driver's own helper de-privileged. `60-nvidia.rules` only ever passes `-c0 -u`, never `-m`, so once BUG-146 stripped setuid from `nvidia-modprobe` **nothing** created that node and every NVIDIA presentation call started failing with a bare `ERROR_UNKNOWN` — that was the real BUG-142. Uses `nvidia-modprobe -m` rather than `mknod` because the minor (254) is a driver convention, not something `/proc/devices` reports. Absent-and-uncreatable is a loud `exit 1` naming the BUG-142 symptom. [CHANGE: claude-code | 2026-08-29]

- `config/lutris-system.yml` — repo copy of `~/.config/lutris/system.yml` (DECISION 90). Two keys: `prefix_command: dgpu-exec-v2 --` and `mangohud: true`. This is what makes Lutris games reach the RTX 4050 at all — without it the DECISION 25 gate denies `/dev/nvidia*` and every game silently renders on the AMD 780M. **Lutris does not read this from the repo** — it is a copy for review and restore; the live file is the one under `~/.config`. Lutris's own precedence is system.yml → runner → game, so any per-game "Command prefix" set in the UI **replaces** this, it does not append. [CHANGE: claude-code | 2026-09-13]
...
- `scripts/luminos-verify` — The whole-stack health check; symlinked from `/usr/local/bin/luminos-verify`. Sections: [1] Go daemons, [2] KCM plugins, [3] dGPU power gating, **[3b] dGPU gate device nodes**, [4] fan/thermal, [5] MCP tooling. Helpers are `ok`/`warn` (increments WARN)/`bad` (increments FAIL); exit is non-zero on any FAIL. **Section [3] and [3b] are contractually read-only and must never wake the dGPU** — sysfs and `stat(2)` only, no `nvidia-smi`, no opening a device node; do not "improve" them by adding one. `[3b]` (BUG-149, added 2026-08-29) stats all five `/dev/nvidia*` nodes expecting `root:dgpu 660`, checks `71-luminos-uvm-gate.rules` is installed (BUG-147), and **fails** if the setuid bit returns to `/usr/bin/nvidia-modprobe` (BUG-146 reopening after an `nvidia-utils` upgrade). An absent node is a FAIL, not a note — printing `absent` as a neutral line next to `gate: OK` is precisely how a missing `/dev/nvidia-modeset` hid for two days, so the modeset case additionally names BUG-149 and the black-screen symptom in its failure text. This is the first command to run if 007 goes black. [CHANGE: claude-code | 2026-08-29]
...
- `scripts/luminos-hyprpm-sync` — Keeps the hyprpm plugins loadable across Hyprland upgrades; called from the `hyprland.start` handler in `~/.config/caelestia/hypr-user.lua` (**not** deployed to /usr/local/bin — referenced by repo path, like luminos-session-recorder). Compares the 40-char commit in `/var/cache/hyprpm/$USER/state.toml` against `hyprctl version`'s running commit; on mismatch runs `hyprpm update` (clones Hyprland for headers, minutes, needs network), then `hyprpm reload -n` either way, then **verifies by reading `hyprctl plugin list` back** — `hyprpm update` prints a green `✔ Loaded <plugin>` per plugin while loading none, so its own output cannot be trusted. Notifies and exits 1 if fewer plugins are loaded than enabled. `flock`ed against concurrent runs; logs to `$XDG_STATE_HOME/luminos-hyprpm-sync.log`. **Session start is the only workable trigger** — hyprpm builds against the *running* compositor, so a pacman hook would rebuild against the version being replaced, and hooks run as root whose hyprpm state is not Shawn's. BUG-111 / DECISION 49. [CHANGE: claude-code | 2026-08-08]
- `scripts/luminos-caelestia-kwin-session` — Install/check/uninstall for the third greeter session, **Luminos (Caelestia on KWin)** (DECISION 63). Additive by design: the Hyprland entry is never touched, so a bad result costs one reboot. Deploys the launcher to `/usr/local/bin`, the session file to `/usr/share/wayland-sessions`, and seven `~/.local/share/applications/luminos-cael-*.desktop` shortcut targets whose `Exec=` is a Quickshell IPC call (Caelestia's own keys use `hyprland_global_shortcuts_v1`, which KWin does not implement). **The registration is `X-KDE-Shortcuts=` in the .desktop file, not `kglobalshortcutsrc`** — that file only stores *overrides*, and an override beats the desktop entry, so the installer deletes stale `_launch` keys. Followed by `kbuildsycoca6 --noincremental` because kglobalaccel discovers shortcuts via `KApplicationTrader`. Keys: Meta+P launcher (Meta+R is Spectacle's), Meta+K dashboard, Meta+N sidebar, Meta+U utilities, Meta+Escape session, Ctrl+Alt+C clear notifications, Meta+Return terminal. BUG-112. [CHANGE: claude-code | 2026-08-09]
- `scripts/luminos-caelestia-kwin` — The session script; `kwin_wayland` runs it and exits when it returns, so this *is* the session. Deliberately **not** `startplasma-wayland` with plasmashell suppressed: that needs a `LUMINOS_SHELL` env var, and `Linger=yes` means the systemd user manager outlives logout and would carry it into a later plain-Plasma login — a Plasma session with no shell and no explanation. Starts polkit-kde, xdg-desktop-portal-kde and the shell; **not** powerdevil (logind owns the lid; powerdevil would fight Caelestia over the brightness keys) so there is no idle screen-off yet. Also **not** `kglobalacceld` — on Plasma 6 `kwin_wayland` owns `org.kde.kglobalaccel` itself and a standalone daemon exits instantly with an empty log. Shell restarts 3× then falls back to a kitty window showing the log (BUG-092: a black screen is not evidence of a crash) — **the budget resets after 60 s of uptime**, because restarts spread across a day of overlay work were otherwise leaving Shawn with a terminal instead of a bar, hours after the last failure, for no visible reason. Also dumps `qdbus6 … /KWin supportInformation` to `kwin-render.log` 8 s in: that is the compositor stating **which renderer it actually chose**, and it is what settled BUG-115 (hardware OpenGL on the 780M, 120 Hz, VRR off) against a session log full of `eglInitialize failed` lines that are merely KWin probing EGL several ways. Logs in `~/.local/state/luminos/caelestia-kwin/{session,shell,sycoca,polkit,portal-kde,kwin-render}.log`. [CHANGE: claude-code | 2026-08-11]
- `scripts/luminos-caelestia-kwin-overlay` — Python. `SRC=/etc/xdg/quickshell/caelestia` → `DST=~/.config/quickshell/caelestia-kwin`. **272 symlinks + 11 patched + 1 added file**, so upstream stays authoritative across package upgrades. `patched_text()` asserts each anchor appears exactly once and `sys.exit`s otherwise, so an upstream change fails the build loudly instead of shipping a stale copy. `--check` is safe to run any time and is wired into the session installer's check.

  The patched files:

  | File | Patch | Why |
  |---|---|---|
  | `services/ShellState.qml` | `forActive`/`componentsForActive` fall back to `instances[0]` **after** the match loop | BUG-113 — and see that entry for why "before" is not enough |
  | `services/Brightness.qml` | `getMonitor("active")` falls back to `monitors[0]` | same null `monitor` |
  | `modules/drawers/ContentWindow.qml` | new `luminosGrab` property; `mask:` goes `null` (whole surface accepts input) while a drawer is open | BUG-116 — the click never *arrived*: Regions leaves the screen middle click-through, so the press went to the app below |
  | `modules/drawers/Interactions.qml` | `onPressed` now also runs `luminosDismissible()` + `luminosOnOpenPanel()` → `luminosClickOutside()` | BUG-116 — nothing *acted* on it either: `HyprlandFocusGrab.onCleared` was the only close-on-click-away path in the whole shell |
  | `modules/dashboard/Content.qml` | tab switch no longer animates through the intermediate pages | BUG-123 — 461–552 ms → 0–52 ms |
  | `modules/dashboard/Performance.qml` | replaces the dead `ServiceRef { service: Gpu }` with a `dgpu` QtObject; adds the VRAM row | BUG-151 |
  | `modules/dashboard/performance/HeroCard.qml` | two **optional** `statusText`/`statusIcon` properties, empty by default | BUG-151 — the card is shared with CPU, so empty must mean "render exactly as upstream" |
  | `modules/launcher/AppList.qml` | `onValuesChanged` resets `currentIndex` via `Qt.callLater(root.luminosResetIndex)` instead of writing it inline | BUG-152 — ScriptModel emits `valuesChanged()` *after* its row signals, so the inline write re-entered `QQuickItemView::layout()` and segfaulted in `moveTo()`. The helper must be a **named** function: `Qt.callLater` de-dupes by identity, so an inline arrow would never coalesce |
  | `modules/launcher/ContentList.qml` | drops `Behavior on implicitWidth`/`implicitHeight` | BUG-152 — same pair, same reason as `dashboard/Content.qml`, but worse: the height chains to the result `count`, so it animated the panel (and the layer-shell surface bound to it) on **every keystroke** |

  **The first four are the same root cause: Caelestia asks Hyprland a question, KWin has no answer, the call returns null, and null silently evaluates false.** No error, no log line — the feature just takes the wrong branch. ⚠️ **The trap that cost the most time:** do **not** patch by reading `focusGrab.active`. `HyprlandFocusGrab` *drops the write* when the protocol is absent, so the property reads back **false forever** and every binding on it freezes — the first version of the BUG-116 patch keyed off it, did nothing, and looked like a QML syntax error. It also makes upstream's own `if (focusGrab.active) return 0;` in `dragMaskPadding` dead code here. Recompute the condition into a local `readonly property` instead. [CHANGE: claude-code | 2026-08-11]

  `NEW_FILES` is the second mechanism: a repo file copied to a path upstream does **not** have. It is not a patch — there is no anchor to match — so the safety check is inverted: if upstream ever ships that path, the build `sys.exit`s rather than silently shadowing it. Currently one entry. [CHANGE: claude-code | 2026-08-31]
- `config/quickshell/caelestia-kwin-extra/VramCard.qml` — installed by the overlay as `modules/dashboard/performance/VramCard.qml`. MemoryCard's twin with one deliberate difference: **it owns no data source.** `awake`/`used`/`total` are all fed in from `Performance.qml`'s `dgpu`, which reads the kernel's `runtime_status` first and only runs `nvidia-smi` while the card is *already* awake. A VRAM widget that polled the GPU itself would hold the 4050 out of D3cold at a measured 13.46 W for as long as the tab is open (BUG-151), so **this file must stay dumb.** `total` is a hardware constant and is therefore *not* cleared on sleep — once the card has woken once, `0 / 6141 MiB` while asleep is the truth, not a guess; before that it prints `Not read yet` rather than a fabricated zero. Follows `gpuCard.active` and has no config toggle of its own, because `Caelestia.Config` is a **compiled QML plugin** and a `showVram` key cannot be added from the overlay. [CHANGE: claude-code | 2026-08-31]
